...
- **`misspell` conflict**: The linter `misspell` enforces US spelling standards. It automatically converted `cancelled` to `canceled`. In `clonepick/clonepick.go`, this broke the `StatusCancelled` JSON API payload value (`"cancelled"` became `"canceled"`), causing a regression in downstream API consumers.
...
- **Fixed API Payload**: Restored `StatusCancelled = "cancelled"` in `clonepick/clonepick.go` to fix the API contract.
