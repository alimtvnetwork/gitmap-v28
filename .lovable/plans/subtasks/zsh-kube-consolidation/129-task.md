...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_129.go"]
depends_on: ["128-task.md"]
citations:
  ...
  tests: "unit TestComp129"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 129 — Implement logic for unit component 129

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 129 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 129. It interacts with specific data structures bound to identifier 6566230e3a3c. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input129 { ID string }`
Output: `struct Output129 { Result bool }`
Emits error codes: E_COMP_129_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_129.go`.
2. Define `func HandleComp129(in Input129) (Output129, error)`.
3. Process data uniqueness string: a30f4ef42176.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp129` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp129`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp129` is implemented according to contract.
