...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_235.go"]
depends_on: ["234-task.md"]
citations:
  ...
  tests: "unit TestComp235"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 235 — Wire and test integration for unit component 235

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 235 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 235. It interacts with specific data structures bound to identifier 0a2d643bfd24. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input235 { ID string }`
Output: `struct Output235 { Result bool }`
Emits error codes: E_COMP_235_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_235.go`.
2. Define `func HandleComp235(in Input235) (Output235, error)`.
3. Process data uniqueness string: 30eec89ddd9c.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp235` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp235`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp235` is implemented according to contract.
