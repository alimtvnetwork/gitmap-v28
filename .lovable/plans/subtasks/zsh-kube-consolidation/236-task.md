...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_236.go"]
depends_on: ["235-task.md"]
citations:
  ...
  tests: "unit TestComp236"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 236 — Wire and test integration for unit component 236

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 236 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 236. It interacts with specific data structures bound to identifier 9a049b03f6fc. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input236 { ID string }`
Output: `struct Output236 { Result bool }`
Emits error codes: E_COMP_236_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_236.go`.
2. Define `func HandleComp236(in Input236) (Output236, error)`.
3. Process data uniqueness string: b6cb293891dd.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp236` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp236`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp236` is implemented according to contract.
