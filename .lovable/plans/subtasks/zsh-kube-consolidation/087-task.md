...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_087.go"]
depends_on: ["086-task.md"]
citations:
  ...
  tests: "unit TestComp087"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 087 — Define spec for unit component 087

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 087 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 087. It interacts with specific data structures bound to identifier bdd2d3af3a5a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input087 { ID string }`
Output: `struct Output087 { Result bool }`
Emits error codes: E_COMP_087_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_087.go`.
2. Define `func HandleComp087(in Input087) (Output087, error)`.
3. Process data uniqueness string: 41e521adf8ae.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp087` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp087`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp087` is implemented according to contract.
