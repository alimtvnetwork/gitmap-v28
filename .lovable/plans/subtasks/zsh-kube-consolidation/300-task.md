...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_300.go"]
depends_on: ["299-task.md"]
citations:
  ...
  tests: "unit TestComp300"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 300 — Wire and test integration for unit component 300

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 300 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 300. It interacts with specific data structures bound to identifier 983bd614bb5a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input300 { ID string }`
Output: `struct Output300 { Result bool }`
Emits error codes: E_COMP_300_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_300.go`.
2. Define `func HandleComp300(in Input300) (Output300, error)`.
3. Process data uniqueness string: 284b7e6d788f.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp300` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp300`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp300` is implemented according to contract.
