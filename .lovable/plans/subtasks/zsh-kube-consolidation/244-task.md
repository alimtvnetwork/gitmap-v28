...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_244.go"]
...
citations:
  ...
  tests: "unit TestComp244"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 244 — Wire and test integration for unit component 244

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 244 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 244. It interacts with specific data structures bound to identifier 82c01ce15b43. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input244 { ID string }`
Output: `struct Output244 { Result bool }`
Emits error codes: E_COMP_244_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_244.go`.
2. Define `func HandleComp244(in Input244) (Output244, error)`.
3. Process data uniqueness string: a77b6cbdf6fa.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp244` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp244`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp244` is implemented according to contract.
