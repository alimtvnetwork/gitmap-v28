...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_130.go"]
depends_on: ["129-task.md"]
citations:
  ...
  tests: "unit TestComp130"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 130 — Implement logic for unit component 130

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 130 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 130. It interacts with specific data structures bound to identifier 38d66d9692ac. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input130 { ID string }`
Output: `struct Output130 { Result bool }`
Emits error codes: E_COMP_130_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_130.go`.
2. Define `func HandleComp130(in Input130) (Output130, error)`.
3. Process data uniqueness string: 39bb88f40d3a.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp130` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp130`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp130` is implemented according to contract.
