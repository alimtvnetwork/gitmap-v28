...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_196.go"]
...
citations:
  ...
  tests: "unit TestComp196"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 196 — Implement logic for unit component 196

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 196 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 196. It interacts with specific data structures bound to identifier b4bbe448fde3. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input196 { ID string }`
Output: `struct Output196 { Result bool }`
Emits error codes: E_COMP_196_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_196.go`.
2. Define `func HandleComp196(in Input196) (Output196, error)`.
3. Process data uniqueness string: 6ea2fdb3399f.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp196` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp196`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp196` is implemented according to contract.
