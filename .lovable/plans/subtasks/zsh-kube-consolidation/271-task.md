...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_271.go"]
depends_on: ["270-task.md"]
citations:
  ...
  tests: "unit TestComp271"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 271 — Wire and test integration for unit component 271

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 271 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 271. It interacts with specific data structures bound to identifier 3635a91e3da8. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input271 { ID string }`
Output: `struct Output271 { Result bool }`
Emits error codes: E_COMP_271_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_271.go`.
2. Define `func HandleComp271(in Input271) (Output271, error)`.
3. Process data uniqueness string: 2d86377d4cc3.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp271` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp271`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp271` is implemented according to contract.
