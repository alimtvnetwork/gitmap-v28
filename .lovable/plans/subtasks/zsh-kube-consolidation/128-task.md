...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_128.go"]
depends_on: ["127-task.md"]
citations:
  ...
  tests: "unit TestComp128"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 128 — Implement logic for unit component 128

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 128 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 128. It interacts with specific data structures bound to identifier 2747b7c71856. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input128 { ID string }`
Output: `struct Output128 { Result bool }`
Emits error codes: E_COMP_128_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_128.go`.
2. Define `func HandleComp128(in Input128) (Output128, error)`.
3. Process data uniqueness string: 51e8ea280b44.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp128` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp128`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp128` is implemented according to contract.
