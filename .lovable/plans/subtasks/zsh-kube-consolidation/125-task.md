...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_125.go"]
...
citations:
  ...
  tests: "unit TestComp125"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 125 — Implement logic for unit component 125

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 125 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 125. It interacts with specific data structures bound to identifier 0f8ef3377b30. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input125 { ID string }`
Output: `struct Output125 { Result bool }`
Emits error codes: E_COMP_125_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_125.go`.
2. Define `func HandleComp125(in Input125) (Output125, error)`.
3. Process data uniqueness string: 1e472b39b105.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp125` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp125`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp125` is implemented according to contract.
