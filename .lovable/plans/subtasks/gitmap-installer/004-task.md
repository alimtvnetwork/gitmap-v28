...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_004.go", "gitmap/cmd/install_comp_004_test.go"]
depends_on: ["Task 003"]
citations:
...
## 2. Goal
Implement `InstallComp004` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input004` struct
Output: `Output004` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `94c34d299adb` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_004.go`.
2. Define `type Input004 struct { Data string }` and `type Output004 struct { Result string }`.
3. Define `func HandleInstallComp004(in Input004) (Output004, error)`.
4. Create `gitmap/cmd/install_comp_004_test.go`.
5. Write unit test `TestInstallComp004` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp004
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp004` exists and passes unit tests.
