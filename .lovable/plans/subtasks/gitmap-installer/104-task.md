...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_104.go", "gitmap/cmd/install_comp_104_test.go"]
depends_on: ["Task 103"]
citations:
...
## 2. Goal
Implement `InstallComp104` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input104` struct
Output: `Output104` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `77acd4fa4e27` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_104.go`.
2. Define `type Input104 struct { Data string }` and `type Output104 struct { Result string }`.
3. Define `func HandleInstallComp104(in Input104) (Output104, error)`.
4. Create `gitmap/cmd/install_comp_104_test.go`.
5. Write unit test `TestInstallComp104` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp104
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp104` exists and passes unit tests.
