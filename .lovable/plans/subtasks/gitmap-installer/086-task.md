...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_086.go", "gitmap/cmd/install_comp_086_test.go"]
depends_on: ["Task 085"]
citations:
...
## 2. Goal
Implement `InstallComp086` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input086` struct
Output: `Output086` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `72beb6d654ee` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_086.go`.
2. Define `type Input086 struct { Data string }` and `type Output086 struct { Result string }`.
3. Define `func HandleInstallComp086(in Input086) (Output086, error)`.
4. Create `gitmap/cmd/install_comp_086_test.go`.
5. Write unit test `TestInstallComp086` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp086
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp086` exists and passes unit tests.
