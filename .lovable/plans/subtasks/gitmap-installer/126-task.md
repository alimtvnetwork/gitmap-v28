...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_126.go", "gitmap/cmd/install_comp_126_test.go"]
depends_on: ["Task 125"]
citations:
...
## 2. Goal
Implement `InstallComp126` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input126` struct
Output: `Output126` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `32ddf87cc144` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_126.go`.
2. Define `type Input126 struct { Data string }` and `type Output126 struct { Result string }`.
3. Define `func HandleInstallComp126(in Input126) (Output126, error)`.
4. Create `gitmap/cmd/install_comp_126_test.go`.
5. Write unit test `TestInstallComp126` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp126
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp126` exists and passes unit tests.
