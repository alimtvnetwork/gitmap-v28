...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_220.go", "gitmap/cmd/install_comp_220_test.go"]
depends_on: ["Task 219"]
citations:
...
## 2. Goal
Implement `InstallComp220` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input220` struct
Output: `Output220` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `1a1b983a05c0` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_220.go`.
2. Define `type Input220 struct { Data string }` and `type Output220 struct { Result string }`.
3. Define `func HandleInstallComp220(in Input220) (Output220, error)`.
4. Create `gitmap/cmd/install_comp_220_test.go`.
5. Write unit test `TestInstallComp220` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp220
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp220` exists and passes unit tests.
