...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_245.go", "gitmap/cmd/install_comp_245_test.go"]
depends_on: ["Task 244"]
citations:
...
## 2. Goal
Implement `InstallComp245` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input245` struct
Output: `Output245` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `44ad5398a405` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_245.go`.
2. Define `type Input245 struct { Data string }` and `type Output245 struct { Result string }`.
3. Define `func HandleInstallComp245(in Input245) (Output245, error)`.
4. Create `gitmap/cmd/install_comp_245_test.go`.
5. Write unit test `TestInstallComp245` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp245
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp245` exists and passes unit tests.
