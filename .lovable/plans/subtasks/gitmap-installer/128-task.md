...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_128.go", "gitmap/cmd/install_comp_128_test.go"]
depends_on: ["Task 127"]
citations:
...
## 2. Goal
Implement `InstallComp128` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input128` struct
Output: `Output128` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `98b23551f9e5` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_128.go`.
2. Define `type Input128 struct { Data string }` and `type Output128 struct { Result string }`.
3. Define `func HandleInstallComp128(in Input128) (Output128, error)`.
4. Create `gitmap/cmd/install_comp_128_test.go`.
5. Write unit test `TestInstallComp128` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp128
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp128` exists and passes unit tests.
