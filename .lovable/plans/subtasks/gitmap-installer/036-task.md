...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_036.go", "gitmap/cmd/install_comp_036_test.go"]
depends_on: ["Task 035"]
citations:
...
## 2. Goal
Implement `InstallComp036` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input036` struct
Output: `Output036` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `7ec4ed9addf7` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_036.go`.
2. Define `type Input036 struct { Data string }` and `type Output036 struct { Result string }`.
3. Define `func HandleInstallComp036(in Input036) (Output036, error)`.
4. Create `gitmap/cmd/install_comp_036_test.go`.
5. Write unit test `TestInstallComp036` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp036
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp036` exists and passes unit tests.
