...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_180.go", "gitmap/cmd/install_comp_180_test.go"]
depends_on: ["Task 179"]
citations:
...
## 2. Goal
Implement `InstallComp180` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input180` struct
Output: `Output180` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `e22712fccde3` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_180.go`.
2. Define `type Input180 struct { Data string }` and `type Output180 struct { Result string }`.
3. Define `func HandleInstallComp180(in Input180) (Output180, error)`.
4. Create `gitmap/cmd/install_comp_180_test.go`.
5. Write unit test `TestInstallComp180` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp180
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp180` exists and passes unit tests.
