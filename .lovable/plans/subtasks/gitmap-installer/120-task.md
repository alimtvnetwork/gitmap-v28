...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_120.go", "gitmap/cmd/install_comp_120_test.go"]
depends_on: ["Task 119"]
citations:
...
## 2. Goal
Implement `InstallComp120` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input120` struct
Output: `Output120` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `032763f5f417` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_120.go`.
2. Define `type Input120 struct { Data string }` and `type Output120 struct { Result string }`.
3. Define `func HandleInstallComp120(in Input120) (Output120, error)`.
4. Create `gitmap/cmd/install_comp_120_test.go`.
5. Write unit test `TestInstallComp120` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp120
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp120` exists and passes unit tests.
