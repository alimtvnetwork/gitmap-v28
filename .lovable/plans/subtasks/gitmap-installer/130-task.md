...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/install_comp_130.go", "gitmap/cmd/install_comp_130_test.go"]
depends_on: ["Task 129"]
citations:
...
## 2. Goal
Implement `InstallComp130` which processes a subset of the installer specifications logic. This task will scaffold the unit, add the interface and strict typing for installer commands, and ensure it builds correctly.

## 3. Inputs and Contracts
Input: `Input130` struct
Output: `Output130` struct
Error: standard `apperror.Result` envelope.
Data Uniqueness: `ac957a2531a2` (for testing).

## 4. Execute
1. Create `gitmap/cmd/install_comp_130.go`.
2. Define `type Input130 struct { Data string }` and `type Output130 struct { Result string }`.
3. Define `func HandleInstallComp130(in Input130) (Output130, error)`.
4. Create `gitmap/cmd/install_comp_130_test.go`.
5. Write unit test `TestInstallComp130` covering success and failure.
...
## 6. Verify
```bash
go test ./cmd/... -run TestInstallComp130
```
Expected output: `PASS` and `ok`.

## 7. Done When
- [ ] `HandleInstallComp130` exists and passes unit tests.
