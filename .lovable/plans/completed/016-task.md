...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_016.go"]
depends_on: ["015-task.md"]
citations:
  ...
  tests: "unit TestComp016"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 016 — Define spec for unit component 016

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 016 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 016. It interacts with specific data structures bound to identifier b17ef6d19c7a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input016 { ID string }`
Output: `struct Output016 { Result bool }`
Emits error codes: E_COMP_016_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_016.go`.
2. Define `func HandleComp016(in Input016) (Output016, error)`.
3. Process data uniqueness string: e29c9c180c62.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp016` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp016`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp016` is implemented according to contract.
