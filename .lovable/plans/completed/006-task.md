...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_006.go"]
...
citations:
  ...
  tests: "unit TestComp006"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 006 — Define spec for unit component 006

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 006 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 006. It interacts with specific data structures bound to identifier e7f6c011776e. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input006 { ID string }`
Output: `struct Output006 { Result bool }`
Emits error codes: E_COMP_006_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_006.go`.
2. Define `func HandleComp006(in Input006) (Output006, error)`.
3. Process data uniqueness string: 6b51d431df5d.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp006` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp006`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp006` is implemented according to contract.
