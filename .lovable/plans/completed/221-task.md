...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_221.go"]
depends_on: ["220-task.md"]
citations:
  ...
  tests: "unit TestComp221"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 221 — Wire and test integration for unit component 221

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 221 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 221. It interacts with specific data structures bound to identifier 67e9c3acebb1. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input221 { ID string }`
Output: `struct Output221 { Result bool }`
Emits error codes: E_COMP_221_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_221.go`.
2. Define `func HandleComp221(in Input221) (Output221, error)`.
3. Process data uniqueness string: 5627b4a8f9ef.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp221` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp221`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp221` is implemented according to contract.
