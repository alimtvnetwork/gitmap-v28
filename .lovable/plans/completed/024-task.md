...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_024.go"]
...
citations:
  ...
  tests: "unit TestComp024"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 024 — Define spec for unit component 024

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 024 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 024. It interacts with specific data structures bound to identifier c2356069e9d1. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input024 { ID string }`
Output: `struct Output024 { Result bool }`
Emits error codes: E_COMP_024_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_024.go`.
2. Define `func HandleComp024(in Input024) (Output024, error)`.
3. Process data uniqueness string: 98010bd9270f.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp024` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp024`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp024` is implemented according to contract.
