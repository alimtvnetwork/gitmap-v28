...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_290.go"]
depends_on: ["289-task.md"]
citations:
  ...
  tests: "unit TestComp290"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 290 — Wire and test integration for unit component 290

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 290 stays within size limits.
...
This task handles the Wire and test integration for of component 290. It interacts with specific data structures bound to identifier 09895de0407b. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input290 { ID string }`
Output: `struct Output290 { Result bool }`
Emits error codes: E_COMP_290_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_290.go`.
2. Define `func HandleComp290(in Input290) (Output290, error)`.
3. Process data uniqueness string: de0023e39811.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp290` under 50 lines.
...
Run `go test ./cmd/... -run TestComp290`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp290` is implemented according to contract.
