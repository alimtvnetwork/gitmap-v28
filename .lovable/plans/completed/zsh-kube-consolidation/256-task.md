...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_256.go"]
depends_on: ["255-task.md"]
citations:
  ...
  tests: "unit TestComp256"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 256 — Wire and test integration for unit component 256

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 256 stays within size limits.
...
This task handles the Wire and test integration for of component 256. It interacts with specific data structures bound to identifier 51e8ea280b44. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input256 { ID string }`
Output: `struct Output256 { Result bool }`
Emits error codes: E_COMP_256_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_256.go`.
2. Define `func HandleComp256(in Input256) (Output256, error)`.
3. Process data uniqueness string: 94f8607915df.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp256` under 50 lines.
...
Run `go test ./cmd/... -run TestComp256`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp256` is implemented according to contract.
