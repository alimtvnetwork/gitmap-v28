...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_280.go"]
...
citations:
  ...
  tests: "unit TestComp280"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 280 — Wire and test integration for unit component 280

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 280 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 280. It interacts with specific data structures bound to identifier 7f0a22117f8f. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input280 { ID string }`
Output: `struct Output280 { Result bool }`
Emits error codes: E_COMP_280_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_280.go`.
2. Define `func HandleComp280(in Input280) (Output280, error)`.
3. Process data uniqueness string: 6bcaea988250.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp280` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp280`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp280` is implemented according to contract.
