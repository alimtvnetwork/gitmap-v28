...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_260.go"]
depends_on: ["259-task.md"]
citations:
  ...
  tests: "unit TestComp260"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 260 — Wire and test integration for unit component 260

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 260 stays within size limits.
...
This task handles the Wire and test integration for of component 260. It interacts with specific data structures bound to identifier 39bb88f40d3a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input260 { ID string }`
Output: `struct Output260 { Result bool }`
Emits error codes: E_COMP_260_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_260.go`.
2. Define `func HandleComp260(in Input260) (Output260, error)`.
3. Process data uniqueness string: 0b35b06a2277.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp260` under 50 lines.
...
Run `go test ./cmd/... -run TestComp260`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp260` is implemented according to contract.
