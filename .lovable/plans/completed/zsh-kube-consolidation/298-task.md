...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_298.go"]
depends_on: ["297-task.md"]
citations:
  ...
  tests: "unit TestComp298"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 298 — Wire and test integration for unit component 298

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 298 stays within size limits.
...
## 2. Goal
This task handles the Wire and test integration for of component 298. It interacts with specific data structures bound to identifier 76ebdb6d45c6. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input298 { ID string }`
Output: `struct Output298 { Result bool }`
Emits error codes: E_COMP_298_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_298.go`.
2. Define `func HandleComp298(in Input298) (Output298, error)`.
3. Process data uniqueness string: be6b5b7140b0.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp298` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp298`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp298` is implemented according to contract.
