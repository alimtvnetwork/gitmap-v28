...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_240.go"]
depends_on: ["239-task.md"]
citations:
  ...
  tests: "unit TestComp240"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 240 — Wire and test integration for unit component 240

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 240 stays within size limits.
...
This task handles the Wire and test integration for of component 240. It interacts with specific data structures bound to identifier 6af1f692e949. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input240 { ID string }`
Output: `struct Output240 { Result bool }`
Emits error codes: E_COMP_240_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_240.go`.
2. Define `func HandleComp240(in Input240) (Output240, error)`.
3. Process data uniqueness string: ddfe0e8d462a.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp240` under 50 lines.
...
Run `go test ./cmd/... -run TestComp240`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp240` is implemented according to contract.
