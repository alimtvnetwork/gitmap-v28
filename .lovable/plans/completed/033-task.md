...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_033.go"]
depends_on: ["032-task.md"]
citations:
  ...
  tests: "unit TestComp033"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 033 — Define spec for unit component 033

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 033 stays within size limits.
...
This task handles the Define spec for of component 033. It interacts with specific data structures bound to identifier c6f3ac57944a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input033 { ID string }`
Output: `struct Output033 { Result bool }`
Emits error codes: E_COMP_033_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_033.go`.
2. Define `func HandleComp033(in Input033) (Output033, error)`.
3. Process data uniqueness string: 3ada92f28b4c.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp033` under 50 lines.
...
Run `go test ./cmd/... -run TestComp033`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp033` is implemented according to contract.
