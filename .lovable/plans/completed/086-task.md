...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_086.go"]
...
citations:
  ...
  tests: "unit TestComp086"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 086 — Define spec for unit component 086

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 086 stays within size limits.
...
This task handles the Define spec for of component 086. It interacts with specific data structures bound to identifier 434c9b5ae514. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input086 { ID string }`
Output: `struct Output086 { Result bool }`
Emits error codes: E_COMP_086_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_086.go`.
2. Define `func HandleComp086(in Input086) (Output086, error)`.
3. Process data uniqueness string: 68519a9eca55.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp086` under 50 lines.
...
Run `go test ./cmd/... -run TestComp086`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp086` is implemented according to contract.
