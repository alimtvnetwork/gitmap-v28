...
domain: Plugin
phase: Implement
target_files: ["gitmap/cmd/os_mirror_detect.go"]
...
# Task 022 — OS Mirror Glitch Detector

## 1. Goal

Detect broken country-specific Ubuntu/Debian mirrors in `cmd/os_mirror_detect.go`.

## 2. Inputs and Contracts

- Package: derived from `cmd/os_mirror_detect.go`
- Strict error wrapping with `apperror`.

## 3. Verify

```bash
go test ./... -run TestOSMirrorGlitchDetector
```
