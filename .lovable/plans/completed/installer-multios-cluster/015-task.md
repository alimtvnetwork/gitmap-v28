...
domain: Plugin
phase: Implement
target_files: ["gitmap/cmd/sshjoin_distribute.go"]
...
# Task 015 — SSH Key Distribute Core

## 1. Goal

Distribute public keys to multiple machines concurrently in `cmd/sshjoin_distribute.go`.

## 2. Inputs and Contracts

- Package: derived from `cmd/sshjoin_distribute.go`
- Strict error wrapping with `apperror`.

## 3. Verify

```bash
go test ./... -run TestSSHKeyDistributeCore
```
