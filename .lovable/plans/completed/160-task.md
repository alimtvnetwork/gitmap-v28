...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_160.go"]
depends_on: ["159-task.md"]
citations:
  ...
  tests: "unit TestComp160"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 160 — Implement logic for unit component 160

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 160 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 160. It interacts with specific data structures bound to identifier a512db2741cd. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input160 { ID string }`
Output: `struct Output160 { Result bool }`
Emits error codes: E_COMP_160_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_160.go`.
2. Define `func HandleComp160(in Input160) (Output160, error)`.
3. Process data uniqueness string: 88820462180e.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp160` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp160`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp160` is implemented according to contract.
