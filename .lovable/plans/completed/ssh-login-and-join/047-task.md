...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/ipchange_windows.go"]
depends_on: [Task 045]
citations:
...
- [gitmap/cmd/ipchange_windows.go](gitmap/cmd/ipchange_windows.go) — Why: Target file.

## 2. Goal

Deliver the Implement step for `swapIPWindows` to support the Implement OS specific IP swap (Windows) feature. This is isolated logic for the SSH/IP subdomains.
...
1. Use `netsh interface ip set address` via `os/exec`.
...
- [ ] 1. `swapIPWindows` is fully functional.
