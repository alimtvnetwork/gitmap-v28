...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_071.go"]
depends_on: ["070-task.md"]
citations:
  ...
  tests: "unit TestComp071"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 071 — Define spec for unit component 071

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 071 stays within size limits.
...
This task handles the Define spec for of component 071. It interacts with specific data structures bound to identifier 7f2253d7e228. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input071 { ID string }`
Output: `struct Output071 { Result bool }`
Emits error codes: E_COMP_071_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_071.go`.
2. Define `func HandleComp071(in Input071) (Output071, error)`.
3. Process data uniqueness string: d4ee9f58e586.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp071` under 50 lines.
...
Run `go test ./cmd/... -run TestComp071`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp071` is implemented according to contract.
