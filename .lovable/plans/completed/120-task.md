...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_120.go"]
...
citations:
  ...
  tests: "unit TestComp120"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 120 — Implement logic for unit component 120

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 120 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 120. It interacts with specific data structures bound to identifier 2abaca4911e6. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input120 { ID string }`
Output: `struct Output120 { Result bool }`
Emits error codes: E_COMP_120_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_120.go`.
2. Define `func HandleComp120(in Input120) (Output120, error)`.
3. Process data uniqueness string: 6af1f692e949.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp120` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp120`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp120` is implemented according to contract.
