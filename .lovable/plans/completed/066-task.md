...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_066.go"]
...
citations:
  ...
  tests: "unit TestComp066"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 066 — Define spec for unit component 066

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 066 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 066. It interacts with specific data structures bound to identifier 3ada92f28b4c. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input066 { ID string }`
Output: `struct Output066 { Result bool }`
Emits error codes: E_COMP_066_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_066.go`.
2. Define `func HandleComp066(in Input066) (Output066, error)`.
3. Process data uniqueness string: dbb1ded63bc7.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp066` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp066`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp066` is implemented according to contract.
