...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_222.go"]
depends_on: ["221-task.md"]
citations:
  ...
  tests: "unit TestComp222"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 222 — Wire and test integration for unit component 222

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 222 stays within size limits.
...
This task handles the Wire and test integration for of component 222. It interacts with specific data structures bound to identifier 9b871512327c. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input222 { ID string }`
Output: `struct Output222 { Result bool }`
Emits error codes: E_COMP_222_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_222.go`.
2. Define `func HandleComp222(in Input222) (Output222, error)`.
3. Process data uniqueness string: 3538a1ef2e11.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp222` under 50 lines.
...
Run `go test ./cmd/... -run TestComp222`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp222` is implemented according to contract.
