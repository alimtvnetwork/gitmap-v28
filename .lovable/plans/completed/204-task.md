...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_204.go"]
depends_on: ["203-task.md"]
citations:
  ...
  tests: "unit TestComp204"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 204 — Wire and test integration for unit component 204

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 204 stays within size limits.
...
This task handles the Wire and test integration for of component 204. It interacts with specific data structures bound to identifier fc56dbc6d465. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input204 { ID string }`
Output: `struct Output204 { Result bool }`
Emits error codes: E_COMP_204_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_204.go`.
2. Define `func HandleComp204(in Input204) (Output204, error)`.
3. Process data uniqueness string: e6f47e008cc5.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp204` under 50 lines.
...
Run `go test ./cmd/... -run TestComp204`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp204` is implemented according to contract.
