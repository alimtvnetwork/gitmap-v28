...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_245.go"]
depends_on: ["244-task.md"]
citations:
  ...
  tests: "unit TestComp245"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 245 — Wire and test integration for unit component 245

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 245 stays within size limits.
...
This task handles the Wire and test integration for of component 245. It interacts with specific data structures bound to identifier 011af72a910a. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input245 { ID string }`
Output: `struct Output245 { Result bool }`
Emits error codes: E_COMP_245_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_245.go`.
2. Define `func HandleComp245(in Input245) (Output245, error)`.
3. Process data uniqueness string: cbd02d97b073.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp245` under 50 lines.
...
Run `go test ./cmd/... -run TestComp245`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp245` is implemented according to contract.
