...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_255.go"]
depends_on: ["254-task.md"]
citations:
  ...
  tests: "unit TestComp255"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 255 — Wire and test integration for unit component 255

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 255 stays within size limits.
...
This task handles the Wire and test integration for of component 255. It interacts with specific data structures bound to identifier 9556b82499cc. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input255 { ID string }`
Output: `struct Output255 { Result bool }`
Emits error codes: E_COMP_255_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_255.go`.
2. Define `func HandleComp255(in Input255) (Output255, error)`.
3. Process data uniqueness string: 5e5c743a015f.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp255` under 50 lines.
...
Run `go test ./cmd/... -run TestComp255`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp255` is implemented according to contract.
