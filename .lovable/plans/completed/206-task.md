...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_206.go"]
depends_on: ["205-task.md"]
citations:
  ...
  tests: "unit TestComp206"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 206 — Wire and test integration for unit component 206

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 206 stays within size limits.
...
This task handles the Wire and test integration for of component 206. It interacts with specific data structures bound to identifier 5cf4e26bd3d8. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input206 { ID string }`
Output: `struct Output206 { Result bool }`
Emits error codes: E_COMP_206_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_206.go`.
2. Define `func HandleComp206(in Input206) (Output206, error)`.
3. Process data uniqueness string: fabf5b7fedb3.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp206` under 50 lines.
...
Run `go test ./cmd/... -run TestComp206`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp206` is implemented according to contract.
