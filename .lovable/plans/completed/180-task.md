...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_180.go"]
depends_on: ["179-task.md"]
citations:
  ...
  tests: "unit TestComp180"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 180 — Implement logic for unit component 180

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 180 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 180. It interacts with specific data structures bound to identifier 7b69759630f8. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input180 { ID string }`
Output: `struct Output180 { Result bool }`
Emits error codes: E_COMP_180_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_180.go`.
2. Define `func HandleComp180(in Input180) (Output180, error)`.
3. Process data uniqueness string: 838f461c2fa6.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp180` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp180`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp180` is implemented according to contract.
