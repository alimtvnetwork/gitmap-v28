...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_007.go"]
depends_on: ["006-task.md"]
citations:
  ...
  tests: "unit TestComp007"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 007 — Define spec for unit component 007

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 007 stays within size limits.
...
This task handles the Define spec for of component 007. It interacts with specific data structures bound to identifier 7902699be42c. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input007 { ID string }`
Output: `struct Output007 { Result bool }`
Emits error codes: E_COMP_007_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_007.go`.
2. Define `func HandleComp007(in Input007) (Output007, error)`.
3. Process data uniqueness string: 8527a891e224.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp007` under 50 lines.
...
Run `go test ./cmd/... -run TestComp007`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp007` is implemented according to contract.
