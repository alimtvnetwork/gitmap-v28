...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_085.go"]
depends_on: ["084-task.md"]
citations:
  ...
  tests: "unit TestComp085"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 085 — Define spec for unit component 085

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 085 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 085. It interacts with specific data structures bound to identifier b4944c6ff08d. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input085 { ID string }`
Output: `struct Output085 { Result bool }`
Emits error codes: E_COMP_085_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_085.go`.
2. Define `func HandleComp085(in Input085) (Output085, error)`.
3. Process data uniqueness string: 734d0759cdb4.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp085` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp085`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp085` is implemented according to contract.
