...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_038.go"]
depends_on: ["037-task.md"]
citations:
  ...
  tests: "unit TestComp038"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 038 — Define spec for unit component 038

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 038 stays within size limits.
...
This task handles the Define spec for of component 038. It interacts with specific data structures bound to identifier aea92132c4cb. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input038 { ID string }`
Output: `struct Output038 { Result bool }`
Emits error codes: E_COMP_038_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_038.go`.
2. Define `func HandleComp038(in Input038) (Output038, error)`.
3. Process data uniqueness string: f74efabef12e.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp038` under 50 lines.
...
Run `go test ./cmd/... -run TestComp038`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp038` is implemented according to contract.
