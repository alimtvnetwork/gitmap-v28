...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_064.go"]
depends_on: ["063-task.md"]
citations:
  ...
  tests: "unit TestComp064"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 064 — Define spec for unit component 064

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 064 stays within size limits.
...
This task handles the Define spec for of component 064. It interacts with specific data structures bound to identifier a68b412c4282. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input064 { ID string }`
Output: `struct Output064 { Result bool }`
Emits error codes: E_COMP_064_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_064.go`.
2. Define `func HandleComp064(in Input064) (Output064, error)`.
3. Process data uniqueness string: 2747b7c71856.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp064` under 50 lines.
...
Run `go test ./cmd/... -run TestComp064`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp064` is implemented according to contract.
