...
domain: Plugin
phase: Implement
target_files: ["gitmap/installer/redo.go", "gitmap/installer/redo_test.go"]
depends_on: ["Task 025"]
citations:
...
## 1. Learn
- Read `.lovable/spec/commands/01-gitmap-installer.md` to understand the overarching requirement for (m *Manager) Redo(slug).
- Read `spec/02-coding-guidelines/00-canonical-size-tier.md` to ensure `gitmap/installer/redo.go` remains concisely sized.
- Review `spec/03-error-manage/02-error-architecture/00-overview.md` for proper `apperror` context wrapping.
- Inspect `gitmap/installer/redo.go` dependencies to see how (m *Manager) Redo(slug) interacts with its callers.

## 2. Goal
The objective is to implement `(m *Manager) Redo(slug)` natively in `gitmap/installer/redo.go`. This explicitly unblocks downstream operations dependent on `Manager Redo Logic` in the Plugin domain. No other files should be manipulated.

## 3. Inputs and Contracts
- Exported Symbols: `(m *Manager) Redo(slug)`
- Package: `installer`
- Error wrapping MUST use the `E_INSTALLER_*` code family.

## 4. Execute
1. Open `gitmap/installer/redo.go`.
2. Implement the required structure, type, or function for `(m *Manager) Redo(slug)`.
3. Write unit tests for success and failure boundaries in `gitmap/installer/redo_test.go`.
...
## 6. Verify
```bash
go test ./installer -run TestManagerRedo
```
Expected output: The test suite passes cleanly with no panics.

## 7. Done When
- [ ] `(m *Manager) Redo(slug)` is successfully mapped and tested in `gitmap/installer/redo.go`.
