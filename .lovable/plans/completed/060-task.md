...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_060.go"]
depends_on: ["059-task.md"]
citations:
  ...
  tests: "unit TestComp060"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 060 — Define spec for unit component 060

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 060 stays within size limits.
...
## 2. Goal
This task handles the Define spec for of component 060. It interacts with specific data structures bound to identifier 39fa9ec190ee. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input060 { ID string }`
Output: `struct Output060 { Result bool }`
Emits error codes: E_COMP_060_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_060.go`.
2. Define `func HandleComp060(in Input060) (Output060, error)`.
3. Process data uniqueness string: 2abaca4911e6.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp060` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp060`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp060` is implemented according to contract.
