...
domain: Spec
phase: Scaffold
target_files: ["gitmap/cmd/comp_030.go"]
depends_on: ["029-task.md"]
citations:
  ...
  tests: "unit TestComp030"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 030 — Define spec for unit component 030

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 030 stays within size limits.
...
This task handles the Define spec for of component 030. It interacts with specific data structures bound to identifier 624b60c58c9d. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input030 { ID string }`
Output: `struct Output030 { Result bool }`
Emits error codes: E_COMP_030_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_030.go`.
2. Define `func HandleComp030(in Input030) (Output030, error)`.
3. Process data uniqueness string: 39fa9ec190ee.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp030` under 50 lines.
...
Run `go test ./cmd/... -run TestComp030`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp030` is implemented according to contract.
