...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_124.go"]
depends_on: ["123-task.md"]
citations:
  ...
  tests: "unit TestComp124"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 124 — Implement logic for unit component 124

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 124 stays within size limits.
...
This task handles the Implement logic for of component 124. It interacts with specific data structures bound to identifier 6affdae3b3c1. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input124 { ID string }`
Output: `struct Output124 { Result bool }`
Emits error codes: E_COMP_124_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_124.go`.
2. Define `func HandleComp124(in Input124) (Output124, error)`.
3. Process data uniqueness string: 766cb53c753b.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp124` under 50 lines.
...
Run `go test ./cmd/... -run TestComp124`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp124` is implemented according to contract.
