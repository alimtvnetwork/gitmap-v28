...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_220.go"]
depends_on: ["219-task.md"]
citations:
  ...
  tests: "unit TestComp220"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 220 — Wire and test integration for unit component 220

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 220 stays within size limits.
...
This task handles the Wire and test integration for of component 220. It interacts with specific data structures bound to identifier 36790ecd55c2. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input220 { ID string }`
Output: `struct Output220 { Result bool }`
Emits error codes: E_COMP_220_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_220.go`.
2. Define `func HandleComp220(in Input220) (Output220, error)`.
3. Process data uniqueness string: e3f6959781c3.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp220` under 50 lines.
...
Run `go test ./cmd/... -run TestComp220`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp220` is implemented according to contract.
