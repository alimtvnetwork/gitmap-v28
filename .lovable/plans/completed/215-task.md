...
domain: Cli
phase: Wire+Test
target_files: ["gitmap/cmd/comp_215.go"]
...
citations:
  ...
  tests: "unit TestComp215"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 215 — Wire and test integration for unit component 215

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 215 stays within size limits.
...
This task handles the Wire and test integration for of component 215. It interacts with specific data structures bound to identifier d86580a57f7b. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input215 { ID string }`
Output: `struct Output215 { Result bool }`
Emits error codes: E_COMP_215_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_215.go`.
2. Define `func HandleComp215(in Input215) (Output215, error)`.
3. Process data uniqueness string: fed88b40aba6.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp215` under 50 lines.
...
Run `go test ./cmd/... -run TestComp215`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp215` is implemented according to contract.
