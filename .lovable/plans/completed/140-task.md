...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_140.go"]
...
citations:
  ...
  tests: "unit TestComp140"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---

# Task 140 — Implement logic for unit component 140

## 1. Learn

- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 140 stays within size limits.
...
This task handles the Implement logic for of component 140. It interacts with specific data structures bound to identifier dbae772db290. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts

Input: `struct Input140 { ID string }`
Output: `struct Output140 { Result bool }`
Emits error codes: E_COMP_140_FAIL

## 4. Execute

1. Create `gitmap/cmd/comp_140.go`.
2. Define `func HandleComp140(in Input140) (Output140, error)`.
3. Process data uniqueness string: 7f0a22117f8f.
4. Return success.

## 5. Constraints

- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp140` under 50 lines.
...
Run `go test ./cmd/... -run TestComp140`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When

1. `HandleComp140` is implemented according to contract.
