...
domain: Cli
phase: Implement
target_files: ["gitmap/cmd/comp_144.go"]
depends_on: ["143-task.md"]
citations:
  ...
  tests: "unit TestComp144"
  ci_cd_guard: "linter-scripts/check-golang.sh"
  ambiguity: "n/a — spec is clear"
  issue_rca: "n/a — not a bug fix"
---
# Task 144 — Implement logic for unit component 144

## 1. Learn
- [Spec](spec/02-coding-guidelines/00-canonical-size-tier.md) - Why read this: ensures component 144 stays within size limits.
...
## 2. Goal
This task handles the Implement logic for of component 144. It interacts with specific data structures bound to identifier 5ec1a0c99d42. It will not mutate global state outside its sandbox.

## 3. Inputs and Contracts
Input: `struct Input144 { ID string }`
Output: `struct Output144 { Result bool }`
Emits error codes: E_COMP_144_FAIL

## 4. Execute
1. Create `gitmap/cmd/comp_144.go`.
2. Define `func HandleComp144(in Input144) (Output144, error)`.
3. Process data uniqueness string: 23c657f2efda.
4. Return success.

## 5. Constraints
- [Rule 1](spec/02-coding-guidelines/00-canonical-size-tier.md) - Keep `HandleComp144` under 50 lines.
...
## 6. Verify
Run `go test ./cmd/... -run TestComp144`.
Expected output: `PASS` and `ok gitmap/cmd`

## 7. Done When
1. `HandleComp144` is implemented according to contract.
