...
| 04 | [04-startup-lifecycle-integration-tests.md](04-startup-lifecycle-integration-tests.md) | "Integration tests using temporary plist files" — plist literally, or per-OS analog? Direct API or shell out to binary? | Per-OS analog (`.desktop` on Linux, plist on macOS, Windows skipped) + direct Go API. Also discovered pre-existing duplicate `withFakeLaunchAgentsDir` symbol — logged for follow-up cleanup, not fixed |
