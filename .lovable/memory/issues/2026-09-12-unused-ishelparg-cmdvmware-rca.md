...
## 2. How it happened
The `Run(args []string)` entrypoint in `cli/cmdvmware/vmware.go` delegates help detection to `checkHelp`, which immediately prints help text and exits if `-h`, `--help`, or `help` is passed. Consequently, the helper function `isHelpArg` was never invoked from any codepath within the package.

## 3. Root Cause
- File: `cli/cmdvmware/vmware.go`
- Lines: 25-27
- Unused private symbol `func isHelpArg(arg string) bool`

## 4. Code Fix
Removed `isHelpArg` from `cli/cmdvmware/vmware.go`.
