...
The CI release pipeline (`.github/workflows/release.yml`) automates the production of 6 cross-compiled binary targets (Windows, Linux, Darwin for amd64/arm64) for both `gitmap` and `cli-updater` when pushing to `release/**` branches or `v*` tags. Artifacts use a versioned naming convention (e.g., `gitmap-v28.56.0-windows-amd64.zip`).

## Pipeline Steps

1. **Resolve version** — from tag name or branch name (`release/v2.56.0` → `v2.56.0`).
2. **Build binaries** — cross-compiles 12 binaries (6 gitmap + 6 cli-updater) with `-ldflags` version embedding.
