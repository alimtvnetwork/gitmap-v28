...
[2.5/4] Generate Windows         (go-winres make — Windows only;
        resources                 see 12-windows-icon-embedding.md)
...
committed (per [12-windows-icon-embedding.md](12-windows-icon-embedding.md)).
...
function Build-Binary {
    ...
    try {
        ...
        # See 12-windows-icon-embedding.md — .syso files are NOT
...
Uses `Resolve-DeployTarget` (see [03-deploy-path-resolution.md](03-deploy-path-resolution.md))
and rename-first deploy (see [04-rename-first-deploy.md](04-rename-first-deploy.md)).
...
- **On Windows**, `go-winres make` MUST run before `go build` to
  ...
  See [12-windows-icon-embedding.md](12-windows-icon-embedding.md).
...
| [20-self-update-app-update.md](../17-consolidated-guidelines/20-self-update-app-update.md) | Consolidated build-script and update workflow guidance |
