# `gitmap-v27 release` must not leak gitmap-specific content into other repos' releases

## Symptom

Running `gitmap-v27 release` inside a non-gitmap repository (e.g. `img-pdf-v2`,
...
1. **Attached `release-version-vX.Y.Z.ps1` / `release-version-vX.Y.Z.sh`
   ...
   `REPO="alimtvnetwork/gitmap-v27"` and `BINARY_NAME="gitmap-v27"`.
   A user who copy-pasted that one-liner would download and install the
   gitmap-v27 binary into the wrong repo's release flow.

2. **Dumped gitmap-v27's `CHANGELOG.md` notes into the release body** because
...
- `gitmap-v27/release/workflowfinalize.go::pushAndFinalize` —
...
- `gitmap-v27/release/workflowgithub.go::uploadToGitHub` — the release body
...
For the canonical gitmap-v27 source repos (`gitmap-v27`, `gitmap-v27`, …):
...
- `gitmap-v27/release/releaseinstallhint.go::ShouldPrintInstallHint` — the
  single source of truth for "is this the gitmap-v27 source repo?".
