...
lock-step with the gitmap-v28 database so every scanned repo or path the user
explicitly registers via `gitmap-v28 code` shows up immediately in the VS Code
...
{
  "name": "gitmap-v28",
  "rootPath": "d:\\wp-work\\riseup-asia\\gitmap-v28",
  "paths": [],
  "tags": [],
  "enabled": true,
  "profile": ""
...
| Field      | Type      | gitmap-v28 behavior                                                  |
|------------|-----------|------------------------------------------------------------------|
| `name`     | string    | DB alias. On first insert: folder basename. Updated by `gitmap-v28 as`. |
...
- API: `gitmap-v28 code <alias> <root> [extra...]` (variadic, additive)
  and `gitmap-v28 code paths add|rm|list <alias> [path]` (explicit).
...
- `gitmap-v28 as <newalias>` only rewrites `name`. Multi-root paths, tags,
...
- Opt-out per scan: `gitmap-v28 scan --no-auto-tags`.
...
If the root directory does not exist, gitmap-v28 reports a clear error
...
If the file does not exist, gitmap-v28 creates it with `[]`. If the parent
directory does not exist, gitmap-v28 returns an error rather than creating
...
### 5.1 `gitmap-v28 scan` — auto-sync (default ON)

After the existing scan + DB upsert phase, gitmap-v28 reads every
`VSCodeProject` row and reconciles `projects.json`:

- New `rootPath` → append entry with gitmap-v28 defaults.
...
### 5.2 `gitmap-v28 code [alias] [path]` — register + open

| Invocation                  | Behavior                                                                 |
|-----------------------------|--------------------------------------------------------------------------|
| `gitmap-v28 code`               | Use git repo root (if inside one) else CWD; alias = folder basename.     |
| `gitmap-v28 code myalias`       | Same path resolution; alias overridden to `myalias`.                     |
| `gitmap-v28 code myalias /path` | Use `/path` (any path, no git requirement); alias = `myalias`.           |
...
### 5.3 `gitmap-v28 as <newalias>` — alias rename mirror

Existing `gitmap-v28 as` flow gains a post-hook: after the DB rename succeeds,
it calls the same projects.json sync helper so the matching `rootPath` row
gets its `name` updated. No new flag.

## 6. Database

Extends the unified gitmap-v28 SQLite DB at the binary path
...
All new strings land in `gitmap-v28/constants/constants_vscode.go`
...
| `code` CLI missing in PATH (gitmap-v28 code)   | actionable install hint above                                            |
...
3. `gitmap-v28 code` inside a git repo → repo root added, alias = basename, VS Code opens.
4. `gitmap-v28 code myalias` → alias overridden to `myalias`, VS Code opens.
5. `gitmap-v28 code myalias D:\anywhere` → non-git path added and opened.
6. `gitmap-v28 as newalias` mirrors `name` change to `projects.json`.
...
### `gitmap-v28 code [alias] [path]`

```
user → gitmap-v28 code [alias] [path]
        │
        ▼
...
- `gitmap-v28/constants/constants_vscode.go` (existing executable discovery constants)
