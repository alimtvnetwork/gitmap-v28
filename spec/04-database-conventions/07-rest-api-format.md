...
All responses MUST use the **Universal Response Envelope** defined in the [Response Envelope Specification](../03-error-manage/02-error-architecture/05-response-envelope/05-response-envelope-reference.md).
...
All examples below conform to the [Universal Response Envelope](../03-error-manage/02-error-architecture/05-response-envelope/05-response-envelope-reference.md).
...
All responses use the [Universal Response Envelope](../03-error-manage/02-error-architecture/05-response-envelope/05-response-envelope-reference.md). Summary:
...
| Reference | Location |
|-----------|----------|
| **Response Envelope Spec** | [04-response-envelope-reference.md](../03-error-manage/02-error-architecture/05-response-envelope/05-response-envelope-reference.md) |
...
| Database naming | [./02-naming-conventions.md](./02-naming-conventions.md) |
...
| `X-Idempotency-Key` | All `POST`, `PUT`, `PATCH` | ULID (26 chars), max 64 chars | Server stores result for `IdempotencyKeyTtlSeconds` (per `02-spec/19-main-worker-service/16-tunable-constants.md` §2.2). Replay returns cached result. |
...
| `Authorization` | Per endpoint auth requirement | `Bearer <token>` | Token format defined per spec (e.g. 02-spec/19/12 for Worker JWT). |
...
- `02-spec/19-main-worker-service/07-core-api-endpoints.md` §1 — first introduced these headers.
- `02-spec/19-main-worker-service/16-tunable-constants.md` §2.2 — TTL + max-length values.
- `02-spec/19-main-worker-service/14-error-codes.md` — error codes for header-validation failures.
- `02-spec/03-error-manage/02-error-architecture/05-response-envelope/` — envelope shape that carries these headers' echoes.
