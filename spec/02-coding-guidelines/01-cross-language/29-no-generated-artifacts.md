...
4. **GitHub Actions Zero Storage (Total Ban on `upload-artifact` in CI)**: Never use `actions/upload-artifact` in CI pipelines. Free accounts are capped at 0.5 GB (500 MB) across the account. CI builds are ephemeral compilation verifications; binary artifacts belong exclusively in GitHub Releases via `release.yml`, never in Actions artifact storage. See `spec/02-coding-guidelines/01-cross-language/30-actions-zero-storage.md`.
