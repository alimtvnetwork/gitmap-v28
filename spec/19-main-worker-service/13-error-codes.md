...
Catalogs every error code referenced by `spec/19-main-worker-service/` and by `spec/14-update/28-worker-push-instruction.md`. Codes are split into two sub-ranges by issuing tier so dumb-AI implementers can tell at a glance which side throws what.
