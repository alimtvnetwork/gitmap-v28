...
> ⚠️ **Repo-specific override:** This generic spec shows `npm ci` as the install step. **In this repo, `npm ci` is forbidden in CI** — see [RCA Issue #1](./17-release-pipeline-issues-rca.md#issue-1--npm-ci-fails-lockfile-out-of-sync). The lockfile is not maintained in sync with `package.json`, so docs-site bundling (if ever enabled here) must use `npm install --no-audit --no-fund` or be performed outside CI and committed as a pre-built artifact.
...
| Reference | Location |
|-----------|----------|
| 🔴 RCA Ledger (read before editing) | [`./17-release-pipeline-issues-rca.md`](./17-release-pipeline-issues-rca.md) |
| Shared conventions | [`./03-shared-conventions.md`](./03-shared-conventions.md) |
| GitHub Release standard | [`./04-github-release-standard.md`](./04-github-release-standard.md) |
| Install script generation | [`./07-install-script-generation.md`](./07-install-script-generation.md) |
| Release body and changelog | [`./14-release-body-and-changelog.md`](./14-release-body-and-changelog.md) |
