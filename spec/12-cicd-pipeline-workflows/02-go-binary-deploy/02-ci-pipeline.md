...
```
sha-check ──┬── lint ──────┬── test (matrix: N suites) ──┬── test-summary ── build (matrix: 6 targets) ── build-summary
             │              │                              │
...
- Version is pinned — never use `@latest`
- The action handles caching automatically
...
- Standard library vulnerabilities → **warn only**
...
**Why `fail-fast: false`?** All suites run to completion even if one fails. This gives a complete picture of what is broken, not just the first failure.
...
- `tee` writes output to both stdout and a file for artifact upload
...
A script parses each suite's `test-output.txt` to extract:
...
After failure reporting, merge all coverage profiles:
...
- Validate build output directories before operating: `test -d "$DIR" || exit 1`
...
*Go binary CI pipeline — updated: 2026-04-10*
