...
Autonomously discovers and deletes stored GitHub Actions artifacts and caches via the GitHub API
across target repositories to enforce the Zero-Storage Actions Mandate and keep account
storage usage at 0.0 GB (staying well within quotas and eliminating waste).

Usage:
  # Purge both artifacts and caches from default repositories:
  ...
  # Purge only caches:
  python 03-ai-scripts/34-purge-github-actions-artifacts.py --caches-only
"""
...
def fetch_cache_batch(repo: str, page: int = 1, per_page: int = 100):
    cmd = [
        "gh", "api",
...
def purge_repo_caches(repo: str, max_workers: int = 8):
    print(f"\n========================================================", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="Purge GitHub Actions storage (artifacts & caches) to maintain 0.0 GB storage usage.")
    ...
    do_artifacts = not args.caches_only
    do_caches = not args.artifacts_only

    for repo in repos:
        if do_artifacts:
            purge_repo_artifacts(repo, max_workers=args.workers)
        if do_caches:
            purge_repo_caches(repo, max_workers=max(4, args.workers // 2))
