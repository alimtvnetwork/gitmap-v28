...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Updated: `02-spec/08-json-schemas/_TODO.md` — `find-next` flipped from `med` to `done`.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec: `02-spec/01-app/111-update-remote-probe.md` documents the probe algorithm, fallback order, and CLI flags.",
  ...
  },
  {
    ...
    items: [
      "**Help coverage at 100%:** all 135 command help files now carry the standardized **Scripting (JSON)** section with a copy-paste `gitmap help --json --filter <cmd>` recipe and a pointer to the published JSON Schema (`02-spec/08-json-schemas/help-json.schema.json`). No command is missing machine-readable docs anymore.",
  ...
  },
  {
    ...
    items: [
      "**Docs UI:** the in-app `/changelog` page now leads with a Help UX tip card surfacing `gitmap help --compact`, `--groups`, `--filter <q>` / `-f`, and `--json` (v5.43.0+), with a direct link to the published JSON Schema (`02-spec/08-json-schemas/help-json.schema.json`).",
  ...
  },
  {
    ...
    items: [
      "**New:** `02-spec/08-json-schemas/help-json.schema.json` formally defines the `gitmap help --json` payload. Contract test `helpjson_jsonschema_contract_test.go` validates runtime output against the schema to prevent drift.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec updated: `02-spec/04-generic-cli/27-fix-repo-command.md` adds a **Backup & undo (v5.40.0+)** section.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec updated: `02-spec/04-generic-cli/27-fix-repo-command.md` has a new **Restrict modes (v5.39.0+)** section. New `applyAllTargetsR` / `rewriteFixRepoFileR` variants in `fixrepo_rewrite.go`; the original `applyAllTargets` / `rewriteFixRepoFile` signatures are preserved as restrict=false wrappers so existing tests stay green.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec updated: `02-spec/04-generic-cli/27-fix-repo-command.md` now has a dedicated **Bare-base scope rule (v5.38.0+)** section documenting the v1→v2 restriction with a worked `gitmap` example.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec: new `02-spec/01-app/112-pull-release-cd.md`.",
  ...
  },
  {
    ...
    items: [
      ...
      "Spec: `02-spec/01-app/110-clone-ssh-flag.md`. Memory: `.ai-memory/memory/features/clone-ssh-flag.md`.",
    ],
  },
  {
    version: "v5.19.0",
  ...
  },
  {
    ...
    items: [
      ...
      "New spec `02-spec/02-app-issues/27-release-body-and-snapshots-gitmap-only.md` + memory `.ai-memory/memory/features/release-gitmap-only-body-and-snapshots.md` document the contract.",
    ],
  },
  {
    version: "v5.15.0",
  ...
  },
  {
    ...
    items: [
      ...
      "RCA: `.ai-memory/memory/issues/2026-05-01-smoke-installer-awk-empty.md`. New project rule (memory): JSON config files in CI shell scripts must be parsed with `jq` or `python3`, never `awk -F'\"'` as the primary path.",
      "Files: `.github/scripts/smoke-installer.sh`, `.ai-memory/memory/issues/2026-05-01-smoke-installer-awk-empty.md`, `src/constants/index.ts` (VERSION → v4.7.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.6.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Files: `gitmap-v28/cmd/clonenextfolderdispatch.go` (new), `clonenextfolderdispatch_test.go` (new — 5 test groups), regex extension in `releaserebase.go`, dispatch wiring in `clonenext.go`, new error/default constants. Spec: `02-spec/01-app/111-cn-folder-arg.md`. Plan: `08-cn-folder-arg-plan.md`.",
  ...
  },
  {
    ...
    items: [
      ...
      "UX deviations from original plan: positional `<lang>...` chosen over `--lang <csv>` (more natural for a scaffolder), and `--cwd` deferred (`cd && gitmap templates init …` covers it). Both deviations recorded in `.ai-memory/memory/plans/05-templates-polish-plan.md` Phase 2 notes.",
  ...
  },
  {
    ...
    items: [
      ...
      "Plan 05 Phase 0 (spec) and Phase 3 (diff) now complete — see `02-spec/01-app/110-templates-polish.md` for the canonical contract covering expanded language coverage, `templates init`, and `templates diff`. Phases 1, 2, and 4 (more langs, `init` scaffolder, README + smoke matrix) remain.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added: RCA `02-spec/02-app-issues/33-stale-binary-clone-folder-url-guard.md` with the full evidence trail proving the deployed binary is stale, why every retry hits the same code path, and the prevention rules.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added: RCA report at `02-spec/02-app-issues/32-docs-ui-vscode-grading-missed-request.md` documenting why the request was missed repeatedly and how that process failure is prevented going forward.",
    ],
  },
  {
    version: "v3.93.0",
    ...
    items: [
      ...
      "Added: RCA report at `02-spec/02-app-issues/31-update-cleanup-phase3-observability-gap.md` documenting the actual root cause, affected files, solution, and validation steps.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `# NOTE:` comment blocks at both deletion sites documenting **why** synchronous cleanup is absent and **where** it now happens, with a pointer to `02-spec/08-generic-update/06-cleanup.md` so future contributors don't 'restore' the noisy call.",
    ],
  },
  {
    version: "v3.28.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Updated specs `02-spec/03-general/02f-self-update-orchestration.md`, `02-spec/08-generic-update/05-handoff-mechanism.md`, and `02-spec/08-generic-update/06-cleanup.md` to document the three-phase model (Phase 1: handoff copy, Phase 2: build/deploy, Phase 3: cleanup handoff to deployed binary).",
      "Regenerated mermaid diagrams `02-spec/08-generic-update/images/handoff-mechanism-flow.mmd` and `02-spec/08-generic-update/images/cleanup-flow.mmd` to show the third handoff explicitly, including the lock-conflict explanation.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added spec `02-spec/07-generic-release/08-pinned-version-install-snippet.md` — rendered snippets, installer flag matrix, release-cutting checklist, and CI test contract.",
  ...
  },
  {
    ...
    items: [
      ...
      "Root cause and prevention documented in `02-spec/08-generic-update/09-winres-icon-constraint.md`.",
    ],
  },
  {
    version: "v2.80.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Created portable `02-spec/pipeline/` documentation folder for cross-AI shareability.",
