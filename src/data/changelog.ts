...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "**Code:** new `gitmap-v27/cmd/fixrepo_backup.go` (`fixRepoBackupSession`, lazy mkdir, idempotent per rel — first observation wins), new `gitmap-v27/cmd/undo.go`, new `gitmap-v27/constants/constants_undo.go`. `rewriteOneFile` split into pure-compute + `persistRewrittenFile` so backup runs strictly BEFORE the disk write. `CmdUndo` / `CmdUndoAlias` wired in `roottooling.go` and registered in `cmd_constants_test.go`. Help text: `gitmap-v27/helptext/undo.md`.",
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.40.0**. Synced `gitmap-v27/constants/constants.go` (`Version = \"5.40.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.40.0\"`).",
    ],
  },
  {
    version: "v5.39.0",
    ...
    items: [
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.39.0**. Synced `gitmap-v27/constants/constants.go` (`Version = \"5.39.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.39.0\"`).",
    ],
  },
  {
    version: "v5.38.0",
    ...
    items: [
      ...
      "Regression tests: `TestApplyAllTargets_BareBase_SkippedAtV3Plus` and `TestApplyAllTargets_BareBase_SkippedAtV4WithV1InTargets` in `gitmap-v27/cmd/fixrepo_rewrite_barebase_test.go` lock in the new behavior. The existing v1→v2 test (`TestApplyAllTargets_BareBase_V1To2`) is unchanged.",
      "Single-line change in `gitmap-v27/cmd/fixrepo_rewrite.go` (`applyAllTargets`): `if n == 1` → `if n == 1 && current == 2`.",
      "Pinned: README pinned-version block + version matrix moved to **v5.38.0**. Synced `gitmap-v27/constants/constants.go` (`Version = \"5.38.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.38.0\"`).",
    ],
  },
  {
    version: "v5.37.0",
    ...
    items: [
      ...
      "New file `gitmap-v27/cmd/rootusagefooter.go` (helper `printUsageFooter`) wired into `printUsage`. Header constant `UsageHeaderFmt` rewritten in `gitmap-v27/constants/constants_cli.go`.",
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.37.0**. Synced `gitmap-v27/constants/constants.go` (`Version = \"5.37.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.37.0\"`).",
    ],
  },
  {
    version: "v5.36.0",
    ...
    items: [
      ...
      "Hardened all four PowerShell wrapper sites: `gitmap-v27/constants/constants_cd.go` (gcd + gitmap fns), `constants_pathsnippet.go` (setup-written profile snippet), `constants_cd_shim.go` (template), and `gitmap-v27/scripts/install.ps1` (both the one-liner profile wrapper and the multi-line shim template).",
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.36.0**. Synced `gitmap-v27/constants/constants.go` (`Version = \"5.36.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.36.0\"`).",
    ],
  },
  {
    version: "v5.35.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.35.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.35.0\"`).",
    ],
  },
  {
    version: "v5.31.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.31.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.31.0\"`).",
    ],
  },
  {
    version: "v5.30.0",
    ...
    items: [
      ...
      "Test fix: registered `CmdPush` / `CmdPushAlias` in `topLevelCmds()` parity registry and added `gitmap-v27/helptext/push.md` to satisfy `TestEveryCmdIDHasHelpFile` (both surfaced after the v5.29.0 push command landed).",
      "Pinned: README pinned-version block + version matrix moved to **v5.30.0**.",
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.30.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.30.0\"`).",
    ],
  },
  {
    version: "v5.29.0",
    ...
    items: [
      ...
      "Added: end-to-end tests in `gitmap-v27/cmd/pushpull_transport_e2e_test.go` cover HTTPS↔SSH conversion + persistence, idempotent no-op, `--ssh` winning conflict, and unrecognised-URL fail-open — using a real `git` binary against a temp bare repo (skipped when git is missing).",
      "Spec: `spec/01-app/111-push-pull-transport-flags.md`.",
      "Pinned: README pinned-version block + version matrix moved to **v5.29.0**.",
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.29.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.29.0\"`).",
    ],
  },
  {
    version: "v5.28.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.28.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.28.0\"`).",
    ],
  },
  {
    version: "v5.27.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.27.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.27.0\"`).",
    ],
  },
  {
    version: "v5.26.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.26.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.26.0\"`).",
  ...
  },
  {
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.24.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.24.0\"`).",
    ],
  },
  {
    version: "v5.23.0",
    ...
    items: [
      "Added: `install.ps1` and `install.sh` now live at the **repository root**. New one-liners: Windows `irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v27/main/install.ps1 | iex` and macOS/Linux `curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v27/main/install.sh | sh`. Shorter, more discoverable — no more `gitmap-v27/scripts/` segment for end users.",
      "Kept: `gitmap-v27/scripts/install.{ps1,sh}` are unchanged source-of-truth (consumed by `go:embed` for `gitmap self-install`, the `release-version` generators, CI workflows, smoke tests, and ~50 spec/memory docs). Root files are byte-identical copies — same checksum contract, same behavior.",
      "Pinned: README pinned-version block + version matrix moved to **v5.23.0**. All 11 install URLs in the README rewritten from `main/gitmap-v27/scripts/install.{ps1,sh}` → `main/install.{ps1,sh}`.",
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.23.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.23.0\"`).",
    ],
  },
  {
    version: "v5.22.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v27/constants/constants.go` (`Version = \"5.22.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.22.0\"`).",
  ...
  },
  {
    ...
    items: [
      ...
      "Files: `gitmap-v27/constants/constants_ssh.go` (new subcommand consts + clipboard messages); `gitmap-v27/cmd/ssh.go` (dispatch); `gitmap-v27/cmd/sshcopy.go` (new — `runSSHCopy`, `writeClipboard`, `resolveClipboardTool`); `gitmap-v27/helptext/ssh.md`.",
    ],
  },
  {
    version: "v5.20.0",
    ...
    items: [
      ...
      "Files: `gitmap-v27/cmd/cloneurlconvert.go` (new — `ConvertURLToSSH`, `ConvertURLToHTTPS`, `httpsToSSHShorthand`, `shorthandToHTTPS`, `sshSchemeToShorthand`, `sshSchemeToHTTPS`, `ensureGitSuffix`); `gitmap-v27/cmd/rootflags.go` (UseSSH/UseHTTPS on CloneFlags, `--ssh`/`--https` registration); `gitmap-v27/cmd/clone.go` (`applyURLSchemeFlags`, wired into `runClone`).",
  ...
  },
  {
    ...
    items: [
      ...
      "Added: canonical command banner at the start of `gitmap release-pending` and `gitmap pull-release` — when a short alias (`rp`, `pr`) is invoked, the resolved canonical name + alias print to stderr first (`→ Running: gitmap release-pending  (alias: rp)`) so it's immediately obvious which pipeline is running. New reusable helper `printCanonicalCmdBanner` in `gitmap-v27/cmd/releasepending.go`.",
      "Files: `gitmap-v27/cmd/releasepending.go` (runReleasePending, rejectVersionArgOnPending, printCanonicalCmdBanner, versionLikeArgPattern); `gitmap-v27/cmd/releasepull.go` (runReleasePull banner call).",
    ],
  },
  {
    version: "v5.18.0",
    ...
    items: [
      "Install scripts (`gitmap-v27/scripts/install.sh` + `gitmap-v27/scripts/install.ps1`) now invoke `<bin_path> setup` as a final non-fatal step after `verify_installation` / `Invoke-InstallVerification`, so a fresh `irm … | iex` or `curl … | sh` lands the user with shell wrapper (`gcd` / `gitmap` profile function) + completions installed — no second manual command. Setup is idempotent (marker `# gitmap shell wrapper v2`); failures print a yellow `(setup auto-run skipped …)` line and continue.",
      "`gitmap cd` self-heals when the shell wrapper isn't loaded: `warnIfNoWrapper` in `gitmap-v27/cmd/setupverify.go` now auto-runs the full `gitmap setup` (not just `InstallCDFunction`) so the very first `gitmap cd <repo>` after a fresh install installs the wrapper + completions automatically. Existing stderr reload tip (`. $PROFILE` / `source ~/.zshrc`) still prints so the user knows why the *next* `cd` will actually move the parent shell. Recover-guarded — a setup panic never breaks the cd output.",
      "Closes the first-run UX gap where users hit `! Shell wrapper not active — 'gitmap-v27 cd' printed the path but cannot change your directory.` because `gitmap setup` was a separate manual step.",
    ],
  },
  {
    version: "v5.17.0",
    ...
    items: [
      ...
      "All four PowerShell wrapper templates now collapse the captured output with `(& $real … | Out-String).Trim()` so `$dest` is always a single trimmed string before `Test-Path` / `Set-Location`: `gitmap-v27/constants/constants_cd.go` (CDFuncPowerShell `gcd` + `gitmap`), `gitmap-v27/constants/constants_pathsnippet.go` (PathSnippetPwshFmt `Invoke-GitmapAndSetLocation`), `gitmap-v27/constants/constants_cd_shim.go` (gitmap.ps1 shim), `gitmap-v27/scripts/install.ps1` (Get-GitmapCommandWrapperBlock + Get-GitmapPowerShellShimContent).",
  ...
  },
  {
    ...
    items: [
      "Release body: `uploadToGitHub` in `gitmap-v27/release/workflowgithub.go` now starts with an empty body and only calls `DetectChangelog()` + `AppendPinnedInstallSnippet` when `ShouldPrintInstallHint(getRemoteURL())` is true. Non-gitmap repos get a tag-only release with an empty body — no more gitmap CHANGELOG.md notes dumped into unrelated release pages.",
      "`release-version-vX.Y.Z.{ps1,sh}` snapshot assets are no longer attached to non-gitmap releases. Those snapshots hard-code `REPO=\"alimtvnetwork/gitmap-v27\"` and `BINARY_NAME=\"gitmap-v27\"`. `pushAndFinalize` in `gitmap-v27/release/workflowfinalize.go` wraps `buildReleaseVersionSnapshots` in the same `ShouldPrintInstallHint` gate.",
  ...
  },
  {
    ...
    items: [
      "Markdown help renderer (`gitmap-v27/render`) now color-codes `# comments` inside code fences in green and credential-like tokens (`API_KEY`, `GITMAP_TOKEN`, etc.) and the `hd` alias in magenta.",
  ...
  },
  {
    ...
    items: [
      ...
      "Synced version constants so the regression test in `src/test/version-sync.test.ts` stays green: `gitmap-v27/constants/constants.go` `Version` `5.10.0` → `5.13.0`, and `src/constants/index.ts` web `VERSION` `v5.10.0` → `v5.13.0`.",
      ...
      "Files: `README.md` (pinned-version block + 4 CI badge URLs), `gitmap-v27/constants/constants.go` (`Version`), `src/constants/index.ts` (`VERSION`), `CHANGELOG.md` (v5.13.0 entry), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.14.0",
  ...
  },
  {
    ...
    items: [
      "Closed a long-standing test desync in `gitmap-v27/cmd/replaceversionparse_test.go::TestPairsForTarget` and `gitmap-v27/cmd/replaceaudit_test.go::TestBuildAuditNeedles`. Both tests passed `current=9` (or implied a 1-digit current) into the function under test, but their hard-coded `want` slices included the literal string `\"gitmap-v27\"` next to a `\"gitmap-v27/v9\"` neighbor — an internally contradictory expectation that drifted whenever the project's own version was bumped past v9. After every width-crossing release the same 4-line edit had to be re-applied by hand; if the edit was missed, the test silently locked in the wrong contract.",
      ...
      "Fix: both tests now (1) bind `target` and `current` to named local constants, (2) build the expected `want` slice with `fmt.Sprintf(\"gitmap-v%d\", current)` / `fmt.Sprintf(\"gitmap-v27/v%d\", current)` instead of hard-coded literals, and (3) assert via the derived values. The captured digit and the input version are now structurally the same variable — desync is no longer expressible.",
      ...
      "Verified the production rewrite engine (`gitmap-v27/cmd/fixrepo_rewrite.go::applyAllTargets`) is already width-safe: it folds targets sequentially with a single-pass byte scan and an ASCII-digit lookahead in `writeOneTokenHit`, so a v9-pass on a `-v12` token correctly skips (next byte `2` is a digit) and the v12-pass replaces. No production code change needed — the bug was purely in test expectations.",
      "Files: `gitmap-v27/cmd/replaceversionparse_test.go` (TestPairsForTarget rewritten + new TestPairsForTargetWidthCrossing, `fmt` import added), `gitmap-v27/cmd/replaceaudit_test.go` (TestBuildAuditNeedles rewritten + new TestBuildAuditNeedlesWidthCrossing), `src/constants/index.ts` (VERSION → v4.12.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.11.0",
    ...
    items: [
      "Added a new `fix-repo gofmt audit (affected .go files only)` step to `.github/workflows/ci.yml` (`lint` job), placed BEFORE the existing global `gofmt -l .` hard-gate. It diffs `gitmap-v27/**/*.go` against the PR base (`github.event.pull_request.base.sha`) or `HEAD~1` for pushes, filters changed files to those containing the canonical fix-repo signature `[A-Za-z0-9._-]+-v[0-9]+`, and runs `gofmt -l` on ONLY that subset.",
  ...
  },
  {
    ...
    items: [
      ...
      "Files: `gitmap-v27/tests/fixrepo_test/gofmt_e2e_test.go` (new), `gitmap-v27/tests/fixrepo_test/fixture_helpers_test.go` (new), `src/constants/index.ts` (VERSION → v4.10.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.9.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Implementation: new `gitmap-v27/cmd/fixrepo_gofmt.go` with `runFixRepoGofmt` (orchestrator + dry-run / empty / lookup guards), `invokeGofmt` (batched exec), and `isGoSourceFile` (extension check). `fixRepoSweepResult` (in `gitmap-v27/cmd/fixrepo_scan.go`) gained a `goFiles []string` field that the per-file branch in `processFixRepoFile` appends to whenever a rewrite landed in a `.go` source. `runFixRepo` (in `gitmap-v27/cmd/fixrepo.go`) now calls `runFixRepoGofmt(result.goFiles, opts)` immediately after `emitFixRepoSummary` and before the exit decision.",
      "New constants in `gitmap-v27/constants/constants_fixrepo.go`: `FixRepoMsgGofmtFmt`, `FixRepoMsgGofmtSkip`, `FixRepoMsgGofmtNoneFmt`, `FixRepoErrGofmtFmt`, `FixRepoErrGofmtMissing` — keeps the no-magic-strings rule honored.",
      ...
      "Files: `gitmap-v27/cmd/fixrepo.go`, `gitmap-v27/cmd/fixrepo_scan.go`, `gitmap-v27/cmd/fixrepo_gofmt.go` (new), `gitmap-v27/constants/constants_fixrepo.go`, `gitmap-v27/constants/constants.go` (Version → 4.4.0), `src/constants/index.ts` (VERSION → v4.8.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.7.0",
    ...
    items: [
      "`installer-smoke` job in `.github/workflows/release.yml` was failing post-release with `Error: Binary not found or not executable at $DEST/gitmap-v27` even though `install.sh` had correctly written the binary to `$DEST/gitmap-cli/gitmap-v27` (canonical layout per `gitmap-v27/constants/deploy-manifest.json`).",
      "Root cause: `.github/scripts/smoke-installer.sh#load_deploy_manifest` parsed `appSubdir`, `binaryName.unix`, and `legacyAppSubdirs` out of `deploy-manifest.json` using `awk -F'\"'`. On the GitHub `ubuntu-latest` runner the embedded double-quote field separator combined with `set -euo pipefail` and process substitution intermittently returned the empty string for `APP_SUBDIR`, collapsing the candidate loop to look for `$DEST//gitmap-v27` and `$DEST/gitmap-v27` — both nonexistent — and reporting a phantom `$BIN=$DEST/gitmap-v27` path.",
  ...
  },
  {
    ...
    items: [
      "`gitmap-v27/cmd/clonefixrepo.go`: replaced direct type assertion `runErr.(*exec.ExitError)` with `errors.As(runErr, &exitErr)` so wrapped exec errors still propagate the child exit code through the `clone-fix-repo` / `clone-fix-repo-pub` pipelines. Satisfies golangci-lint `errorlint` and keeps exit-code fidelity for chained steps.",
      "`gitmap-v27/cmd/visibilityapply.go`: collapsed `func parseVisibilityOutput(_ string, out string) string` to `func parseVisibilityOutput(_, out string) string` per gocritic `paramTypeCombine`.",
      "`gitmap-v27/constants/constants_visibility.go`: spelling fix `centralised` → `centralized` per misspell linter.",
      "Files: `gitmap-v27/cmd/clonefixrepo.go`, `gitmap-v27/cmd/visibilityapply.go`, `gitmap-v27/constants/constants_visibility.go`, `src/constants/index.ts` (VERSION → v4.6.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.5.0",
    ...
    items: [
      ...
      "Help & UI: top-level `gitmap help` now lists `clone-fix-repo` and `clone-fix-repo-pub` in the Utilities group; the `--compact` line is updated; web docs (`src/data/commands.ts`) gain full entries with examples and bidirectional `seeAlso` cross-links between `cfr`, `cfrp`, `clone`, `fix-repo`, and `make-public`. Shell completion's generated list (`gitmap-v27/completion/allcommands_generated.go`) picks up `cfr`, `cfrp`, `clone-fix-repo`, and `clone-fix-repo-pub`.",
      "New files: `gitmap-v27/cmd/clonefixrepo.go`, `gitmap-v27/constants/constants_clonefixrepo.go`, `gitmap-v27/helptext/clone-fix-repo.md`, `gitmap-v27/helptext/clone-fix-repo-pub.md`. Edited: `gitmap-v27/cmd/rootcore.go` (dispatch), `gitmap-v27/cmd/rootusageflags.go` (utilities help), `gitmap-v27/constants/constants_helpgroups.go` (compact line), `gitmap-v27/constants/cmd_constants_test.go` (uniqueness test entries), `src/constants/index.ts` (VERSION → v4.5.0), `src/data/commands.ts` (UI entries), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.4.0",
    ...
    items: [
      ...
      "Files: `src/constants/index.ts` (VERSION → v4.4.0), `src/data/changelog.ts` (this entry), plus the prior-step changes to `gitmap-v27/cmd/releasepull.go`, `gitmap-v27/helptext/release-pull.md`, `gitmap-v27/cmd/rootusageflags.go`, `gitmap-v27/constants/constants_fixrepohelp.go`, and `src/data/commands.ts`.",
    ],
  },
  {
    version: "v3.119.0",
    ...
    items: [
      ...
      "Files: `gitmap-v27/cmd/inject.go` (new), `gitmap-v27/helptext/inject.md` (new), `gitmap-v27/constants/constants_cli.go` (CmdInject + CmdInjectAlias), `gitmap-v27/constants/constants_v331.go` (Msg/Warn/Err constants), one dispatch entry in `gitmap-v27/cmd/rootcore.go`. Reuses `resolveCloneNextFolder` for path resolution so error messages stay consistent with `cn <folder>`.",
    ],
  },
  {
    version: "v3.118.0",
    ...
    items: [
      ...
      "File: `gitmap-v27/cmd/clone.go` — one `WriteShellHandoff(absPath)` call inside `executeDirectClone`, placed after the DB upsert + Desktop registration but before the VS Code open so the cd happens regardless of whether VS Code is installed.",
    ],
  },
  {
    version: "v3.117.0",
    ...
    items: [
      ...
      "Files: `gitmap-v27/cmd/clonenextfolderdispatch.go` (new), `clonenextfolderdispatch_test.go` (new — 5 test groups), regex extension in `releaserebase.go`, dispatch wiring in `clonenext.go`, new error/default constants. Spec: `spec/01-app/111-cn-folder-arg.md`. Plan: `08-cn-folder-arg-plan.md`.",
  ...
  },
  {
    ...
    items: [
      ...
      "Three copy-pasteable verification commands confirm the v3.92.0+ rename fix is present: (1) `grep '^const Version = '` against `constants.go` must be 3.92.0 or newer; (2) `grep -nE '^func (fileExists|fileExistsLoose)\\('` against `updatedebugwindows.go` must produce no output (helper moved to `gitmap-v27/fsutil` in v3.113.0); (3) `test -f gitmap-v27/fsutil/exists.go` + import check on both cmd/ files must print both paths.",
  ...
  },
  {
    ...
    items: [
      ...
      "New `gitmap-v27/cmd/updatedebugwindows_source_test.go` parses `updatedebugwindows.go` with `go/parser` and asserts two invariants. `TestUpdateDebugWindowsHasFsutilLooseCall` — the file imports the canonical fsutil path AND contains at least one real call expression of the form `fsutil.FileOrDirExists(...)`. The check uses the AST (selector expression), not a substring scan, so comments mentioning the name do not satisfy the invariant.",
  ...
  },
  {
    version: "v3.113.0",
    date: "2026-04-24",
    subtitle: "Centralize `fileExists` / `fileExistsLoose` / `dirExists` into a shared `gitmap-v27/fsutil` package",
    items: [
      ...
      "New package `gitmap-v27/fsutil` (`gitmap-v27/fsutil/exists.go`) exports three predicates with explicitly documented contracts: `FileExists` (strict file, rejects directories), `FileOrDirExists` (loose existence, accepts directories — replaces the prior `fileExistsLoose`), and `DirExists` (strict directory, rejects files). All three short-circuit on the empty string so callers don't need to guard their inputs.",
      "Contract-pinning tests in `gitmap-v27/fsutil/exists_test.go` exercise every variant against a real tempdir, file, missing path, and the empty string. Collapsing two variants now fails a test before it fails a downstream caller.",
      "`gitmap-v27/cmd/updaterepo.go` — removed local `dirExists` and `fileExists`, calls `fsutil.DirExists` / `fsutil.FileExists` instead. `gitmap-v27/cmd/updatedebugwindows.go` — removed local `fileExistsLoose`, calls `fsutil.FileOrDirExists` instead.",
      "`gitmap-v27/cmd/updatedebugwindows_rename_test.go` — repurposed from the v3.112.0 rename pin into a forward-looking guard. The new `TestFsutilMigrationPinned` asserts the cmd package uses `fsutil.*` and exercises both the loose-empty short-circuit and the strict-dir rejection. A contributor reintroducing a local helper would trigger the redeclaration error this test was created to prevent.",
      "Scope: only the `cmd` package is migrated in this release because that is where the redeclaration risk has materialized. The duplicated helpers in `gitmap-v27/release`, `gitmap-v27/lockfile`, `gitmap-v27/localdirs`, `gitmap-v27/vscodepm`, and `gitmap-v27/detector` are functionally fine (different packages = different namespaces) and can migrate to `fsutil` opportunistically.",
  ...
  },
  {
    ...
    items: [
      "Added typed CLI constants for templates subcommand aliases — `CmdTemplatesDiff` (`\"diff\"`), `CmdTemplatesDiffAlias` (`\"td\"`), `CmdTemplatesInit` (`\"init\"`), `CmdTemplatesInitAlias` (`\"ti\"`) — in `gitmap-v27/constants/constants_templates_cli.go`. The const block opts in via `// gitmap-v27:cmd top-level` so the completion generator picks the alias values up automatically on the next `go generate ./completion/...` run.",
      "Generator-aware skip markers on the full subcommand strings: `CmdTemplatesDiff` and `CmdTemplatesInit` carry `// gitmap-v27:cmd skip` line comments. `\"diff\"` would be a no-op (already a top-level command — folder-tree diff via `gitmap-v27/cmd/diff.go`); `\"init\"` would mislead users into typing `gitmap init` standalone. Both decisions are now documented in source instead of in a side-channel memory note.",
      "`gitmap-v27/cmd/templatesdiff.go` and `gitmap-v27/cmd/templatescli.go` now alias the local `cmdTemplatesDiff*` / `cmdTemplatesInit*` package identifiers to the shared `constants.CmdTemplates*` values, giving the completion generator one source of truth and preventing string drift between the dispatcher switch and the completion table.",
      "Regenerated `gitmap-v27/completion/allcommands_generated.go` — added `\"td\"` and `\"ti\"` in their sorted positions (between `task`/`temp-release` and `tk`/`tpl` respectively). Run `go generate ./completion/...` locally to confirm byte-equality before tagging.",
  ...
  },
  {
    ...
    items: [
      "Verified: `gitmap templates init <lang> [<lang>...] [--lfs] [--dry-run] [--force]` (alias `ti`) is fully implemented in `gitmap-v27/cmd/templatesinit.go` (297 lines), wired into the dispatcher in `templatescli.go`, covered by 9 unit tests in `gitmap-v27/cmd/templatesinit_test.go`, and documented in `gitmap-v27/helptext/templates-init.md` (133 lines).",
  ...
  },
  {
    ...
    items: [
      ...
      "Verified: `gitmap-v27/templates/corpus_parity_test.go` already enumerates the new five langs (lines 23-30) and asserts every ignore lang has a matching attributes counterpart (line 97 loop). Any future revert to the asset directory will fail CI before merge — no `corpus_test.go` extension required.",
      "Verified: no lang enum drift. The resolver discovers languages via filesystem walk + filename parsing per Plan 04 design, so adding a new lang requires only dropping the file in. Confirmed zero references to `LangJava` / `LangRuby` / etc. in `gitmap-v27/templates/*.go`. `templates list` picks up the new langs automatically.",
  ...
  },
  {
    ...
    items: [
      ...
      "Flags: `--lang <name>` (limit to one language, default = every resolvable lang), `--kind ignore|attributes` (default = both), `--cwd <path>` (run against a different working tree). Five unit tests in `gitmap-v27/templates/diff_test.go` pin all four status branches plus blank-line preservation.",
      ...
      "Bumped `constants.Version` to `3.108.0`. New helptext at `gitmap-v27/helptext/templates-diff.md` with the full exit-code table and pre-commit example.",
    ],
  },
  {
    version: "v3.107.0",
    ...
    items: [
      "Extended `gitmap-v27/render/testdata/pretty/` with three new golden-file cases (`007` headings without subtitles, `008` back-to-back collapsible blocks, `009` blank-line preservation) so the renderer's remaining parser branches are pinned by byte-equality before the next round of helptext additions.",
  ...
  },
  {
    ...
    items: [
      ...
      "Fixed: `gitmap-v27/cmd/clone.go` `executeDirectClone` now refuses URL-shaped folder names early with an actionable message that names the exact recovery commands: `gitmap doctor`, `gitmap update`, `gitmap pending clear --yes`, and the reminder to open a NEW terminal so PATH refreshes. This shape is impossible in current source, so the guard fires only when a stale binary is in use — and the message tells the user exactly that, instead of letting git fail with `Invalid argument`.",
      "Added: `gitmap-v27/cmd/clone_stale_binary_test.go` pins all three reported PowerShell argv shapes (`url1,url2,url3` comma-glued, `url1 url2 url3` PowerShell-split, comma+space mixed) plus the URL detector and the recovery message contents. Regressions in any of the three cooperating predicates now fail CI before the broken binary can ship.",
  ...
  },
  {
    ...
    items: [
      ...
      "Fixed: `gitmap-v27/cmd/updatecleanup_remove.go` now mirrors `glob_error`, per-attempt `remove_retry`, final `remove_fail`, and `remove_ok` into the durable handoff log and the optional `--debug-windows-json` NDJSON sink, so per-file cleanup failures survive even when the detached child console output does not.",
      "Fixed: `gitmap-v27/cmd/updatecleanup_extra.go` now mirrors the previously ad-hoc special-case cleanup branches too — `drive_root_skip`, `drive_root_remove_fail` / `drive_root_remove_ok`, `swap_glob_error`, and `swap_remove_fail` / `swap_remove_ok` now produce durable events instead of stderr-only lines.",
      ...
      "Added: `gitmap-v27/cmd/updatecleanup_handoff_test.go` covering stable handoff-log formatting plus forwarding of `--debug-windows`, `--debug-windows-json`, `GITMAP_UPDATE_CLEANUP_DELAY_MS`, and `GITMAP_DEBUG_WINDOWS_JSON` into the cleanup child. Targeted `go test ./cmd ...` validation passed.",
    ],
  },
  {
    version: "v3.92.0",
    ...
    items: [
      "Fixed: duplicate `fileExists` declaration in `gitmap-v27/cmd/` blocked `go test ./cmd/...`. Renamed the debug-dump copy in `updatedebugwindows.go` to `fileExistsLoose` (file-or-dir, empty-string-safe) and updated its two callers in `dumpDebugWindowsHandoff`. The original `fileExists` in `updaterepo.go` keeps its strict file-only semantics.",
      "Added: `gitmap-v27/cmd/root_url_shortcut_test.go` pins the bare-URL shortcut against the three exact `Unknown command` invocations users keep reporting — `gitmap https://a,https://b,https://c` (PowerShell paste), `gitmap https://a, https://b, https://c` (bash paste, comma-then-space), and `gitmap https://a, https://b https://c` (mixed separators). Also covers single-URL, leading-flag (`--verbose <url>`), SSH-shorthand (`git@github.com:a/b.git`), and GitLab variants, plus negative cases (known subcommand, folder path, empty argv) so the shortcut never grabs a legitimate command. Three test functions, all sub-second, no external deps.",
  ...
  },
  {
    ...
    items: [
      "Added `gitmap unzip-compact` (alias `uzc`) — extracts a local archive, an HTTP(S) URL, or auto-picks the single archive in the current folder, into a single normalized output folder under `<dest>/<archive-base-name>/`. The compact-extract algorithm (`gitmap-v27/archive/extract.go`) uses the temp-dir-then-move strategy: stream-extract into a sibling `.gitmap-uzc-*` temp dir, walk to find the deepest single-child directory chain (capped at `MaxCompactFlattenLayers=4` per the spec's 2..4 tolerance), then promote that real root into the final folder. Result: `xap.zip → xap/xap/<files>` collapses to `dest/xap/<files>`, `xlt.zip → <files>` (no folder) wraps cleanly into `dest/xlt/<files>`, and `mixed.zip → README + src/` lands as `dest/mixed/{README,src}` (no flatten possible).",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v27/scripts/install.sh` `download_asset()` now tracks a `strict=1` flag whenever `${VERSION}` is non-empty and routes every failure path through a new `strict_fail()` helper that prints the canonical message and exits 1. The `.zip` alternate-asset fallback (which previously kicked in when the requested `.tar.gz` wasn't listed in `checksums.txt`) is now SKIPPED in strict mode — the tag's own `checksums.txt` is treated as the source of truth for what assets the user asked for. Same logic mirrored in `gitmap-v27/scripts/install.ps1` `Get-Asset` via a new `Stop-Strict` function. Non-strict installs (no `--version` supplied) keep the existing fallback behavior unchanged so brand-new releases that only ship a `.zip` continue to work.",
  ...
  },
  {
    ...
    items: [
      ...
      "Documented the new flag in `gitmap-v27/helptext/changelog.md` (note about `--pretty | less -R` / `--no-pretty` for redirects), updated `gitmap-v27/helptext/templates.md` to list three opt-outs/opt-ins (`--pretty`, `--no-pretty` aliasing legacy `--raw`, and `GITMAP_NO_PRETTY` env), and surfaced the flag in the `templates show` usage block. The `EnvNoPretty = \"GITMAP_NO_PRETTY\"` constant is now exported from `render` so all three surfaces (and any future ones) share a single source of truth for the env var name.",
    ],
  },
  {
    version: "v3.23.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `helptext.PrintRaw(command)` for callers that want to bypass the pretty pipeline unconditionally (future paginator integration, doc generators, snapshot tests). The default `helptext.Print(command)` keeps the same signature so every existing caller in `gitmap-v27/cmd/` upgrades without modification.",
      "Decision recorded in `.lovable/memory/plans/04-templates-ignore-attributes-plan.md`: `gitmap changelog` stays on its existing structured renderer (`renderChangelogEntry` in `gitmap-v27/cmd/changelogprint.go`). That renderer operates on `release.ChangelogEntry` structs with depth-aware bullets, ordered-list markers, hanging indent, wrap width, and inline `**bold**` / `` `code` `` ANSI — strictly richer than what generic markdown rendering can produce. Routing it through `render.RenderANSI` would be a regression. The pretty renderer stays scoped to free-form markdown sources (helptext today; future doc commands).",
    ],
  },
  {
    version: "v3.17.0",
    ...
    items: [
      ...
      "Added paired-fixture corpus in `gitmap-v27/render/testdata/pretty/case-NNN-*.{in.md,want.txt}`: case-001 collapses a redundant fence, case-002 highlights two quoted spans, case-003 covers heading + subtitle + indent, case-004 verifies a non-matching fence is preserved (no false-positive collapse), case-005 mixes all rules. Adding a new edge case is now a two-file drop — no test code changes.",
      "Added `gitmap-v27/render/pretty_test.go`: `TestPrettyFixtures` table-loops over every `*.in.md` in the directory and fails when any pair drifts. `TestRenderANSISwapsTokens` guards against sentinel-token leakage in CLI output. `TestUnterminatedQuoteClosedDefensively` ensures a stray `\"` can't bleed an open ANSI sequence into the rest of the terminal session.",
    ],
  },
  {
    version: "v3.16.0",
    ...
    items: [
      "Added `gitmap templates` subcommand router (alias `tpl`) in `gitmap-v27/cmd/templatescli.go`, wired into `dispatch()` in `root.go` (alongside the previously-missed `dispatchAdd` hook, now also re-confirmed). Subcommands: `list` (alias `tl`) and `show <kind> <lang>` (alias `ts`).",
      ...
      "Added `gitmap-v27/templates/list.go` (+ `dirfs.go` empty-FS fallback): walks the embedded `assets/` first, then overlays user entries from `~/.gitmap/templates/` (silent skip when the dir doesn't exist). The merge map is keyed `<kind>/<lang>` so user files always shadow embeds.",
      "Added `gitmap-v27/constants/constants_templates_cli.go`: `CmdTemplates`/`tpl`, sub-aliases `tl`/`ts`, format strings, error strings, and the user-facing `UsageTemplatesRoot` block.",
      "Added `gitmap-v27/templates/list_test.go`: locks in (1) the embedded corpus shows up with `SourceEmbed`, (2) entries are sorted by kind-rank then lang, (3) a user-overlay file flips `SourceEmbed` → `SourceUser` for the matching `<kind>/<lang>`, and (4) `parseRelTemplatePath` correctly rejects wrong-extension/unknown-kind inputs.",
    ],
  },
  {
    version: "v3.15.0",
    ...
    items: [
      "Added `gitmap add attributes [langs...]` (alias `aa`) in `gitmap-v27/cmd/addattributes.go` — mirror of `add ignore` but writes to `./.gitattributes` using the dedicated `# >>> gitmap-attributes (do not edit between markers) >>>` / `# <<< gitmap-attributes <<<` markers. Same single-pass dedupe, atomic write, and `Changed=false` no-op on re-run.",
  ...
  },
  {
    ...
    items: [
      "Added `gitmap add` subcommand router (`gitmap-v27/cmd/rootadd.go`, `gitmap-v27/constants/constants_add.go`) wired into `dispatch()` in `root.go`. Top-level alias `ad`. Routes to `add ignore` (alias `ai`), `add attributes` (alias `aa`, Phase 3), and `add lfs-install` (alias `alfs`, Phase 3).",
      ...
      "Added `gitmap-v27/templates/merge.go` engine: single-pass dedupe keyed on trimmed line (comments dedupe too), audit-trail headers (`# source:` / `# kind:` / `# lang:` / `# version:`) stripped before merge, blank-line collapse, idempotent — re-running with identical args yields a byte-identical file (`MergeResult.Changed=false`).",
      "Seeded curated corpus in `gitmap-v27/templates/assets/` (Phase 1): `ignore/{common,go,node,python,rust,csharp}.gitignore`, `attributes/{common,go,node,python,rust,csharp}.gitattributes`, and `lfs/common.gitattributes`. Common attributes normalize `*.svg` to `text eol=lf` (NOT LFS — SVG is XML and diffs cleanly); `*.bat`/`*.cmd`/`*.ps1` keep CRLF; raster/archive formats forced binary.",
      "Added `gitmap-v27/templates/{embed.go,resolver.go,materialize.go,paths.go}` (Phase 0): `//go:embed all:assets`, `~/.gitmap/templates/` user overlay (overlay > embed resolution order) so users on read-only install paths (e.g. `C:\\Program Files\\gitmap-v27`) can still customize, idempotent first-run materializer that skips any already-present file.",
  ...
  },
  {
    ...
    date: "2026-04-20",
    items: [
      "Added pinned-version install snippet auto-appended to every GitHub release body (`gitmap-v27/release/installsnippet.go`), with PowerShell + bash one-liners that hard-code the published tag. Idempotent via `<!-- gitmap-pinned-install-snippet:<tag> -->` marker.",
  ...
  },
  {
    ...
    date: "2026-04-19",
    items: [
      "Fixed v15 Phase 1.4 migration crash on databases first created at v3.5.0+: `GoProjectMetadata` and `PendingTask` rebuilds threw `no such column: Id` because both tables were already singular before v15, so the canonical `CREATE TABLE IF NOT EXISTS` pass had already produced the v15-shaped table (with `{Table}Id` PK). New `adaptOldColumnList()` in `gitmap-v27/store/migrate_v15rebuild.go` detects the existing PK column via `columnExists()` and rewrites the leading `Id` token in `OldColumnList` to `{Table}Id` when needed — idempotent, no-op for genuine legacy paths.",
      "Fixed `go vet` `non-constant format string` warning in `gitmap-v27/movemerge/finalize.go:50` by reshaping `logErr(prefix, msg string)` to accept a pre-formatted message and moving `fmt.Sprintf(constants.ErrMMPushFailFmt, sha)` to the call site.",
      "Fixed `gitmap-v27/store/migrations.go` build error caused by an orphan `\"github.com/alimtvnetwork/gitmap-v27/gitmap-v27/constants\"` import.",
      ...
      "Fixed `cd` / `go` constant collision — removed duplicate `CmdCDCmd` / `CmdCDCmdAlias` from `constants_cli.go` and repointed `gitmap-v27/cmd/rootdata.go` dispatch at the canonical `CmdCD` / `CmdCDAlias` constants in `constants_cd.go`.",
      "Added `gitmap-v27/cmd/cmdconstants_unique_test.go` (+ helpers): parses every `constants_*.go` using the same `gitmap-v27:cmd top-level` / `gitmap-v27:cmd skip` markers as the completion generator and fails the suite when two distinct `Cmd*` identifiers claim the same string value. Catches redeclarations and dispatch shadowing at CI time before the build phase.",
      ...
      "Added `gitmap help probe` and `gitmap help sf` docs (synopsis, flags, examples, 3–8 line realistic terminal simulation) in `gitmap-v27/helptext/`.",
  ...
  },
  {
    ...
    items: [
      ...
      "`go-winres` CI failure fixed — moved `winres.json` to `gitmap-v27/winres/` where `go-winres make` expects it.",
  ...
  },
  {
    ...
    items: [
      ...
      "Standardized lowercase \"gitmap-v27\" branding across all installer output.",
  ...
  },
  {
    ...
    items: [
      ...
      "Release pipeline `dist` directory fix — explicit `working-directory: gitmap-v27/dist` prevents `cd: dist` failure in CI.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `gitmap-v27/Makefile` as a thin wrapper around `run.sh` for Linux/macOS.",
