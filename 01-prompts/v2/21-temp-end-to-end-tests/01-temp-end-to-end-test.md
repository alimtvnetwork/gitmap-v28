# Temporary End-to-End Tests & Isolated On-Demand Validation — Workflow (must follow)

Trigger Keywords & Aliases: `temp e2e`, `temp end to end`, `local e2e test`, `isolated e2e test`, `temporary e2e test`, `01-temp-end-to-end-test`
...
1. If `.agents/skills/temp-end-to-end-tests/skill.md` does not exist in the workspace, create it now.
...
To rapidly discover existing test fixtures, integration hooks, configuration files, and services without hitting 50-result tool caps, use the repository's dedicated Python discovery scripts:

- Inventory Target Files: `python 03-ai-scripts/11-fast-file-scanner.py --lang go,ts,py --limit 100 --stats`
...
Execute temporary end-to-end test tasks via a strict 3-Phase pipeline. Do not skip steps.
...
3. Actionable Deliverables Extraction: Break down whatever user requirements were given into discrete, actionable items with ordered traceable IDs (`Task-01`, `Task-02`, `Task-03`) under `## Extracted Actionable Task List`.
...
### Phase 1B: Planning Mode, Canonical Spec & Lean Subtasks

#### Step 1: Canonical Application Spec Generation (Folder 21 Standard)

Write the canonical specification into `02-spec/21-app/` before creating execution plans or modifying code:
- **Location & Sizing Standard:**
  ...
  - Large / multi-domain features: Write to a segmented directory `02-spec/21-app/xx-<slug>/` with sequential sub-files:
    - `01-overview.md` (System context, integration topology, verbatim user request)
    - `02-data-contracts.md` (Models, payload fixtures, schemas, interfaces)
...
- **Parent Plan:** Write `.ai-memory/plans/pending/xx-<slug>.md` containing:
  ...
  - Subsystem topology, involved components, and blast radius analysis.
  - Complete mapping of deliverables (`Task-01`, `Task-02`, etc.) to subtask files.
- **Subtask Files:** Break down the plan into granular subtasks in `.ai-memory/plans/subtasks/xx-<slug>/01-<subtask>.md`, `02-<subtask>.md`, etc.
- **No Common Boilerplate:** Subtask files must contain only the unique items required for that specific subtask.

Subtasks must follow this lean template:
```markdown
...
Targeted Verification: [Specific on-demand command with isolation flags]
```

#### Step 3: Unconditional Zero-Question Execution Mandate

As soon as Phase 1 planning completes, the agent MUST NOT pause, stop, or ask the user confirmation questions. There is NO question. It must immediately, unconditionally transition directly into Phase 2 execution mode.
