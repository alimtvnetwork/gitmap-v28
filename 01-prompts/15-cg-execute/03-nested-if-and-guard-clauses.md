...
14. [ ] /learn Ingest `spec/02-coding-guidelines/01-cross-language/01-index.md` for domain-specific architectural specifications.
...
- Context Diet: When spawning a subagent, DO NOT paste file contents, memory logs, or the entire plan into its prompt. Give it the absolute minimal instruction (e.g., "Read subtask file `.lovable/plans/subtasks/XX-nested-if/01-<subtask-title>.md` and execute it"). The subagent MUST read the necessary files itself.
