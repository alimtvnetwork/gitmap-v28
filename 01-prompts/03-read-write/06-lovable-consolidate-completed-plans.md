...
/goal Autonomously create a timestamped backup branch, scan, analyze, cluster, aggressively consolidate, and re-sequence all completed plan files and subtasks within `.ai-memory/plans/` into minimal, hyper-compact milestone summaries, combining 2, 3, or more related tasks and common checklists into single files to drastically reduce total file count while strictly preserving 100% of core architectural concepts, verified task outcomes, error contracts, and decision logs until 100% green without stopping.
...
4. [ ] /goal Phase 1 (Step A): Deeply scan `.ai-memory/plans/completed/`, `.ai-memory/plans/subtasks/`, and `02-spec/25-app-spec-audit/` to inventory all fragmented micro-plans, subtask files, and completed audit reports.
...
```text
PHASE_1_STEPS = N / 2   (Steps 1 .. N/2: Create Backup Branch, Scan Plans & Subtasks, Cluster by Domain, Spec Compaction in .ai-memory/plans/pending/)
...
>    Record the backup branch name and recovery instructions in `.ai-memory/plans/pending/XX-completed-plans-consolidation.md`:
>    ```bash
...
A project that runs dozens of autonomous agent loops quickly accumulates scores of micro-task files and subtask folders in `.ai-memory/plans/completed/` and `.ai-memory/plans/subtasks/`. If left unmanaged, this file clutter exhausts context windows, bloats search indexes, slows down memory retrieval, and fragments project history.
...
Every consolidated file generated inside `.ai-memory/plans/completed/` MUST adhere to this uniform, high-density layout:
...
- [`.ai-memory/memory/issues/XX-rca.md`](.ai-memory/memory/issues/XX-rca.md) — Root cause analysis and resolution details.
```
...
2. **Inventory Scan:** List all files currently in `.ai-memory/plans/completed/`, `.ai-memory/plans/subtasks/`, and `02-spec/25-app-spec-audit/`. Count total files before compaction.
...
4. **Master Compaction Spec:** Write `.ai-memory/plans/pending/XX-completed-plans-consolidation.md` containing the full clustering table:
...
1. **Write Consolidated Milestone Documents:** Author each consolidated file in `.ai-memory/plans/completed/` following the Compact Milestone Template. Prune zero-business-logic guideline tasks.
...
3. **Clean Removal of Merged Sources:** Use `git rm` on all superseded micro-plan files in `.ai-memory/plans/completed/`.
...
- [ ] **Completed Plans & Subtasks Scanned:** Recursively inspected all files in `.ai-memory/plans/completed/` and `.ai-memory/plans/subtasks/`. Recorded baseline file count.
...
- [ ] **Master Compaction Spec Written:** Created `.ai-memory/plans/pending/XX-completed-plans-consolidation.md` containing the mapping ledger table and backup rollback recipe.
...
- [ ] **Single Master Checklist Mandate:** Verified that all planned milestone summaries reference `.ai-memory/coding-guidelines.md` as the single authoritative checklist without duplicating rules.
...
- [ ] **High-Density Milestone Files Created:** Authored unified milestone summaries matching the Compact Milestone Template in `.ai-memory/plans/completed/`.
...
- [ ] **Checklist Compaction:** Common checklists are consolidated into a single unified reference to `.ai-memory/coding-guidelines.md` per milestone file, avoiding multi-file duplication.
...
- [ ] Master Guidelines: I have fully read and strictly enforced `02-spec/02-coding-guidelines/08-file-folder-naming/` and `.ai-memory/coding-guidelines.md`.
...
- [ ] Monotonic Sequence: Verified sequential `01-`, `02-`, `03-` numbering across `.ai-memory/plans/completed/`.
