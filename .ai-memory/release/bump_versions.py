...
def update_readme_md(current: str, new_version: str):
    ...
    repl_text = (
        ...
        f'**Unix/Bash:** `curl -sL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v28/v{new_version}/install.sh | bash -s -- ".ai-memory/prompts" "v{new_version}"`\n'
        f'**PowerShell:** `Invoke-WebRequest -Uri https://raw.githubusercontent.com/alimtvnetwork/gitmap-v28/v{new_version}/install.ps1 -OutFile install.ps1; .\\install.ps1 -TargetDir ".ai-memory/prompts" -Version "v{new_version}"`'
    )
    content = re.sub(
...
def assemble_release_notes(new_version: str) -> str:
    notes_path = os.path.join(ROOT_DIR, ".ai-memory", "release", f"release-notes-v{new_version}.md")
