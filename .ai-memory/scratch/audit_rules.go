...
func main() {
	...
	filepath.Walk(rootDir, func(path string, info os.FileInfo, err error) error {
		...
		if strings.Contains(path, "vendor") || strings.Contains(path, "node_modules") || strings.Contains(path, ".git") || strings.Contains(path, ".ai-memory") {
			return nil
		}
	...
	os.MkdirAll("d:/wp-work/riseup-asia/gitmap/.ai-memory/plans/subtasks/01-coding-guideline-fixes", 0755)
...
func writeTask(filename, title string, matches []string) {
	...
	path := filepath.Join("d:/wp-work/riseup-asia/gitmap/.ai-memory/plans/subtasks/01-coding-guideline-fixes", filename)
