...
All AI agents triggering releases MUST follow `02-spec/09-pipeline-extend-v2/readme.md` strictly.
...
- `02-spec/01-app/104-clone-multi.md` — add a "Folder naming" rule: folder = repo name from URL verbatim (`-vN` preserved); explicit second positional arg overrides; no flattening in `clone`.
- `.ai-memory/memory/` — record the constraint "base `clone` must never strip `-vN` from folder names" so it is not reintroduced.
