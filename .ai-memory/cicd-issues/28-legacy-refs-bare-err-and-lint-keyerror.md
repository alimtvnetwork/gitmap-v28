...
1. `Legacy Refs Check`: `check-legacy-refs.sh` identified 4 historical mentions of `gitmap-v6` in `.ai-memory/cicd-issues/20-cicd-four-headed-hydra.md` and `.ai-memory/cicd-issues/index.md`. <!-- gitmap-legacy-ref-allow -->
...
1. `.ai-memory/cicd-issues/20-cicd-four-headed-hydra.md` documented a past bug fix where `gitmap-v6` references were replaced. Because `check-legacy-refs.sh` runs grep across all files without `gitmap-legacy-ref-allow` markers, mentioning historical version names in CI logs tripped the check.
...
1. Appended `<!-- gitmap-legacy-ref-allow -->` to historical lines in `.ai-memory/cicd-issues/20-cicd-four-headed-hydra.md` and `.ai-memory/cicd-issues/index.md`.
...
4. Generated `03-ai-scripts/03-cicd-local-runner.py` and verified all checks pass with exit code 0.
