...
Reverted `gitmap/go.mod` back to `go 1.24.13` and ran `go mod tidy`. Used `03-ai-scripts/02-cicd-local-runner.py` to run `golangci-lint` locally and verified that the lint and vet steps now pass with 0 errors.
