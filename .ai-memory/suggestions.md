...
### Author `02-spec/01-app/v4-breaking-change-matrix.md`
...
- **Status:** Pending
- **Priority:** Medium
- **Description:** Decide keep/remove for the `Id` → `{Table}Id` PK rename migration on the same v4.0 schedule. Document decision in `.ai-memory/memory/`.
- **Added:** v3.12.1 session
...
- **Status:** Pending
- **Priority:** Low
- **Description:** Delete stale `02-spec/01-app/gitmap-database-erd.mmd` (v3.5.0) + `gitmap-core-schema-simplified.mmd` (v15 Phase 1.2). Rename `gitmap-database-erd-v3.12.1.mmd` → `gitmap-database-erd.mmd`.
- **Added:** v3.12.1 session
...
- **Status:** Pending
- **Priority:** Medium
- **Description:** Add a Go test under `gitmap/store/` that walks every `SQLCreate*` constant and asserts the table+column list matches `02-spec/01-app/gitmap-database-erd*.mmd` so the ERD cannot drift silently from the schema.
- **Added:** v3.12.1 session

### Create dedicated `02-spec/02-database/` section

- **Status:** Pending
- **Priority:** Low
- **Description:** Move all `*.mmd` ERDs into `02-spec/02-database/`, add an `index.md`, and write per-table contracts (purpose, lifecycle, owning command).
- **Added:** v3.12.1 session
...
- **Status:** Pending
- **Priority:** Medium
- **Description:** MySQL, MariaDB, PostgreSQL, SQLite, MongoDB, CouchDB, Redis, Cassandra, Neo4j, Elasticsearch, DuckDB, Chocolatey, Winget. Full table in `.ai-memory/plan.md` Part B.
...
- **Status:** Pending
- **Priority:** Medium
- **Description:** Per-package-manager uninstall builders + DB record cleanup. Spec in `.ai-memory/plan.md` Part E.
...
### Create `02-spec/01-spec-authoring-guide/` with spec writing conventions

- **Status:** Pending
- **Priority:** Low
- **Description:** Document the spec authoring conventions used across `02-spec/`.
...
- **Implemented:** v3.12.1 session
- **Notes:** Walks `constants/*.go` AST, asserts every `// gitmap:cmd top-level` const block (minus `// gitmap:cmd skip`) is present in the runtime command registry. Marked implemented in `02-spec/01-app/99-cli-cmd-uniqueness-ci-guard.md`.

### v15 legacy compat shim audit (decision record)

- **Implemented:** v3.12.1 session
- **Notes:** `.ai-memory/memory/02-v15-legacy-compat-audit.md` — KEEP both `ReadReleaseMeta` JSON overlay and `migrate_v15phase5` SQLite column rename through v3.x; remove in v4.0.0 with 5-step plan documented.

### Fresh 28-table ERD for v3.12.1

- **Implemented:** v3.12.1 session
- **Notes:** `02-spec/01-app/gitmap-database-erd-v3.12.1.mmd` covers all 28 current tables (11 missing from v3.5.0 ERD added: ScanFolder, Alias, RepoVersionHistory, VersionProbe, etc.). Reflects v15 `IsDraft`/`IsPreRelease` rename.
...
### `02-spec/12-consolidated-guidelines/` with 18 unified guideline documents
...
- **Implemented:** v3.0.0
- **Notes:** Auto-stash labeled `gitmap-release-alias autostash <alias>-<version>-<unix-ts>`, popped via label-match against `git stash list` for concurrent safety. Files: `cmd/{as,asops,releasealias,releasealias_git}.go`. Spec: `02-spec/01-app/98-as-and-release-alias.md`.
...
### Spec doc `02-spec/01-app/98-as-and-release-alias.md`
