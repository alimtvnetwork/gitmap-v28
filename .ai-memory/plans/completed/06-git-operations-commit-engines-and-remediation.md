...
See .ai-memory/plans/subtasks/01-commit-commands-overhaul/ for detailed subtasks.
...
##### 3. Subtasks (to be created in .ai-memory/plans/subtasks/02-git-rm-and-folder/)
...
The previous executions left temporary python scripts (`fix_*.py`) in the root directory, which were incorrectly committed and tracked by git. These need to be moved to `.ai-memory/temp-scripts/`, removed from git tracking, and `.gitignore` needs to be updated to ensure this doesn't happen again.
...
2. **Move**: Move these files into the `.ai-memory/temp-scripts/` directory.
3. **Git Hygiene**:
   - Remove the files from git tracking using `git rm`.
   - Verify that `.ai-memory/temp-scripts/` (or `.ai-memory/temp-scripts`) is explicitly ignored in `.gitignore`. If not, add it.
4. **Release Bump**:
...
- [x] Temp Script Sandboxing: strictly enforce `.ai-memory/temp-scripts/` gitignore rules.
...
Subtasks generated in: `.ai-memory/plans/subtasks/03-fix-release-tag-ordering/01-task.md`
...
##### 3. Subtasks (to be created in .ai-memory/plans/subtasks/03-ignore-and-add/)
...
- **Authoritative Specifications Implemented:**
  - [`02-spec/01-app/04-workspace/01-move-and-remove.md`](02-spec/01-app/04-workspace/01-move-and-remove.md) — Safe repository moving with VS Code & GitHub Desktop sync.
  - [`02-spec/01-app/06-chrome-profile/01-profile-management.md`](02-spec/01-app/06-chrome-profile/01-profile-management.md) — Chrome user profile copying, exporting, and importing.
  - [`02-spec/01-app/07-lfs/01-lfs-smudge.md`](02-spec/01-app/07-lfs/01-lfs-smudge.md) — Resilient LFS clone fallback on missing credentials or offline remotes.
- **Core Architecture Contracts:**
...
- [`.ai-memory/memory/issues/06-sqlite-locking-during-move.md`](.ai-memory/memory/issues/06-sqlite-locking-during-move.md) — SQLite busy timeout during multi-repo moves.
...
1. **Subtask 1: Documentation & Guidelines Update**
   ...
   - Inject the new Error Architecture rules (the `apperror` returning rules) into `.ai-memory/coding-guidelines.md` and `02-spec/02-coding-guidelines/00-overview.md`.
...
3. **Subtask 3: Final Release**
   - Bump version using `.ai-memory/release/bump_versions.py --type minor` (or fallback script).
...
**Parent Plan:** [.ai-memory/plans/pending/64-remediation-fix-and-chrome-token-export.md](../../pending/64-remediation-fix-and-chrome-token-export.md)
**Status:** COMPLETED
**Files:**
...
##### Success Criteria
- [x] Initial extraction writes `last_commit_hash` to `.ai-memory/temp/git-changed-files.json`.
...
1. **Scenario 1: Fresh Run**:
   ...
   - Verify `.ai-memory/temp/git-changed-files.json` contains `last_commit_hash` equal to `git rev-parse HEAD`.
...
- [`.ai-memory/memory/learned/06-macro-step-execution-and-shell-open.md`](.ai-memory/memory/learned/06-macro-step-execution-and-shell-open.md)
- [`.ai-memory/memory/issues/2026-09-04-commit-right-untracked-path.md`](.ai-memory/memory/issues/2026-09-04-commit-right-untracked-path.md)
