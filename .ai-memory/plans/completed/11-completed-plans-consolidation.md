...
- **Completed Plan Markdown Files:** 95 files in `.ai-memory/plans/completed/`
- **Subtask Files in Pending Subtasks:** 282 files across 72 folders in `.ai-memory/plans/subtasks/`
- **Subtask Files in Completed Subtasks:** 20 files across 4 folders in `.ai-memory/plans/completed/subtasks/`
...
1. **Step A — Author High-Density Consolidated Milestones:** Author `01-` through `10-` in `.ai-memory/plans/completed/` following the Standard Compact Milestone Template.
2. **Step B — Clean Removal of Superseded Plans & Subtasks:** Execute `git rm` on all 95 superseded micro-plan files in `.ai-memory/plans/completed/` and all 76 subtask folders in `.ai-memory/plans/subtasks/` and `.ai-memory/plans/completed/subtasks/`.
3. **Step C — Monotonic Re-Sequencing:** Confirm monotonic `01-` through `10-` continuous numbering with strictly lowercase filenames.
4. **Step D — Index Synchronization:** Update `.ai-memory/plans/01-index.md` and `.ai-memory/what-to-read.md`.
...
1. **Mandatory Audit Folder Purge:** When architectural audits are completed, temporary audit directories (`02-spec/21-app/25-app-spec-audit/`, `02-spec/19-main-worker-service/audit/`, `02-spec/21-app/26-coding-guideline-audit/`, `.ai-memory/audits/`) must be backed up to OS temp directory and recycled/purged via `03-ai-scripts/33-git-history-tracer-and-purger.py`.
2. **Guideline Checklist Consolidation:** Milestone files must never re-copy entire coding guideline sections. Milestones reference `.ai-memory/coding-guidelines.md` as a single source of truth.
