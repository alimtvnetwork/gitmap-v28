...
4. **[Pending]** Write RCA to `.ai-memory/cicd-issues/16-github-actions-yaml-parse-fatal.md`.
5. **[Pending]** Update `.ai-memory/cicd-issues/index.md` and `.ai-memory/strictly-avoid.md`.
...
10. **[Pending]** Update `.ai-memory/memory/release-architecture-map.md`.
...
- **Solution in `gitmap/cmd/cg_worker.go`**:
  ...
  - After all workers finish, print a nicely formatted summary block with Lipgloss:
    ...
    - Indicate what files were updated (e.g., `.ai-memory/coding-guidelines.md` and `version.json`).
...
This plan finalizes the local CI/CD quality gate architecture across all 33 gates enqueued in `03-ai-scripts/06-cicd-local-runner.py` (and `03-ai-scripts/06-cicd-local-runner.py`). It implements real-time failure streaming to `.ai-memory/temp/cicd/`, instant terminal failure dumps with stack traces and suspect file lists, and verifies that the complete test suite passes with `exit 0`.

##### 2. Task-Specific Rules & Constraints

1. **Rule 1 (Real-Time Failure Streaming):** All quality gate failures must immediately append to `.ai-memory/temp/cicd/errors.log`, `.ai-memory/temp/cicd/errors.json`, and `.ai-memory/temp/cicd/run.log` without waiting for the test suite to complete.
...
###### A. Real-Time Telemetry Files (`.ai-memory/temp/cicd/`)
...
- [ ] All 33 quality gates in `03-ai-scripts/06-cicd-local-runner.py` and `03-ai-scripts/06-cicd-local-runner.py` pass cleanly (`exit 0`).
- [ ] Real-time failures stream immediately to `.ai-memory/temp/cicd/errors.log` and `.ai-memory/temp/cicd/errors.json`.
...
##### Objective
Refactor `03-ai-scripts/06-cicd-local-runner.py` and `03-ai-scripts/06-cicd-local-runner.py` to:
1. Append all gate outcomes to `CICD_RUN_LOG` (`.ai-memory/temp/cicd/run.log`).
2. Continuously update `CICD_SUMMARY_JSON` (`.ai-memory/temp/cicd/summary.json`) with active counts (`total_gates`, `passed_gates`, `failed_gates`, `remaining_gates`, `status`).
...
##### Files
- `03-ai-scripts/06-cicd-local-runner.py`
- `.ai-memory/temp/cicd/summary.json`
- `.ai-memory/temp/cicd/run.log`
...
This plan upgrades `03-ai-scripts/06-cicd-local-runner.py` and its synchronized mirror `03-ai-scripts/06-cicd-local-runner.py` into an intelligent, crash-resilient, incremental test runner. The enhanced runner retains phase execution state, fingerprinting inputs (git commit SHA, uncommitted working tree deltas, linter script timestamps, and configuration files) to skip previously passing, unchanged gates in O(1) time (~0.5ms per gate). It features a multi-session storage architecture (`.ai-memory/temp/cicd/runs/<timestamp>/` and `.ai-memory/temp/cicd/latest/`), dual-mode parallel telemetry with in-flight worker slot tracking, zero-delay terminal failure reporting with suspect file extraction, and automatic crash resumption.
...
###### A. Session Scaffolding & Storage Subsystem (`.ai-memory/temp/cicd/`)
- Isolated run directories under `.ai-memory/temp/cicd/runs/<timestamp>/`.
- Symlink / Directory Junction at `.ai-memory/temp/cicd/latest/` with fallback to `latest_run.txt`.
...
4. [04-runner-verification-and-resumption-test.md](subtasks/68-smart-incremental-cicd-runner/04-runner-verification-and-resumption-test.md): Full execution verification, caching validation, `--force` test, and sync to `03-ai-scripts/06-cicd-local-runner.py`.
...
- [x] Session runs are persisted under `.ai-memory/temp/cicd/runs/<timestamp>/` with updated `state.json`.
...
1. **Multi-Session Isolation**:
   - Create timestamped run directory: `.ai-memory/temp/cicd/runs/<timestamp>/`.
   - Maintain symlink / Windows junction / pointer at `.ai-memory/temp/cicd/latest/` pointing to the active session.
   - Maintain backward-compatible mirrors at `.ai-memory/temp/cicd/` root: `errors.log`, `errors.json`, `run.log`, `summary.json`, `state.json`.
...
##### Acceptance Criteria
- [x] Session directory `.ai-memory/temp/cicd/runs/<timestamp>/` created on startup.
...
3. **AI Agent Instructions Header**:
   - Update runner module docstring with clear instructions explaining real-time failure streaming, how to monitor `.ai-memory/temp/cicd/`, and parallel remediation.
4. **Coding Guidelines**:
   - All functions $\le 15$ lines.
   - Blank line before every return statement.
   - Affirmative booleans (`is_*`, `has_*`).
...
##### Objective
Verify the end-to-end execution of all 33 quality gates, validate incremental skipping speed, test `--force` cache bypass, verify crash resumption, and synchronize changes to `03-ai-scripts/06-cicd-local-runner.py`.
...
2. **Mirror Synchronization**:
   - Ensure exact 1:1 match between `03-ai-scripts/06-cicd-local-runner.py` and `03-ai-scripts/06-cicd-local-runner.py`.
3. **Plan Indexing**:
   - Move completed subtasks to completed directories and update `.ai-memory/plans/01-index.md`.
4. **Coding Guidelines**:
...
This plan elevates `03-ai-scripts/06-cicd-local-runner.py` (and `03-ai-scripts/06-cicd-local-runner.py`) into a truly unbuffered, parallel-remediation CI/CD streaming engine. It guarantees that any test or quality gate failure is immediately broadcast to the terminal and streamed to `.ai-memory/temp/cicd/` without OS buffer delays using `os.fsync`. It introduces an append-only line-delimited `events.jsonl` stream and `changelog.log`, enhances suspect file extraction across all 33 gates (including Windows drive letters, TypeScript `(line,col)`, Python tracebacks, and semantic git delta fallbacks), displays `Working Dir` and `Env Overrides` in terminal failure banners, provides an end-of-run `AI AGENT REMEDIATION SUMMARY & LOG LOCATIONS` dashboard, and equips the runner with an Active Operational Instruction Manual in the module header for AI agents working in parallel.

##### 2. Task-Specific Rules & Constraints

1. **Rule 1 (Strict No Automatic Releases):** Under NO circumstances bump versions in `version.json`, edit changelogs, or cut release tags. Commits must remain standard development commits.
2. **Rule 2 (Strict Relative Paths):** Zero `file:///` URIs or machine-specific absolute paths in code, plans, or telemetry. All paths must be relative to repository root (`/`).
3. **Rule 3 (Unbuffered fsync Disk Flushing):** All file writes to `.ai-memory/temp/cicd/` (`errors.log`, `run.log`, `events.jsonl`, `changelog.log`) must call `flush()` and `os.fsync(fh.fileno())` under a write lock so concurrent tailing agents see every byte instantaneously.
...
###### A. Universal Synchronized Disk Streaming Engine (`.ai-memory/temp/cicd/`)
...
##### Objective
Update the runner module docstring into an Active Operational Instruction Manual for AI agents, run end-to-end quality gate verification across all 33 gates, validate incremental skip performance, audit coding guidelines, and synchronize `03-ai-scripts/06-cicd-local-runner.py`.

##### Requirements
1. **Module Docstring Active Operational Header**:
   - Section 1: Parallel Execution Model & 3-Batch Barrier Pipeline.
   - Section 2: Real-Time Telemetry & Artifact Streaming (`.ai-memory/temp/cicd/`).
...
4. **Mirror Sync & Plan Closure**:
   - Synchronize `03-ai-scripts/06-cicd-local-runner.py` to `03-ai-scripts/06-cicd-local-runner.py`.
   - Update `.ai-memory/plans/01-index.md` and move Plan 69 to completed.
   - Zero automatic releases or version bumps.
...
2. **CI/CD Pipeline Python Runner (`03-ai-scripts/06-cicd-local-runner.py`):**
   ...
   - **Test Inventory Manifest (`.ai-memory/test-inventory.json`):**
...
3. **Bounded Folders:** All logs, plans, and test caches are stored within `.ai-memory/`.
...
- [02-task-cicd-test-inventory-generator.md](../subtasks/81-cicd-smart-worker-groups-and-install-ls/02-task-cicd-test-inventory-generator.md): Implement test discovery in `06-cicd-local-runner.py` generating `.ai-memory/test-inventory.json` with execution timings.
...
3. Verify test inventory generation: run `python 03-ai-scripts/06-cicd-local-runner.py --inventory-only` and inspect `.ai-memory/test-inventory.json`.
...
##### 1. Description
Implement a test inventory generator at the very start of `03-ai-scripts/06-cicd-local-runner.py` that discovers all existing tests in the codebase (all Go unit tests `Test*` across `*_test.go`, linter gates, and E2E suites), catalogs them into `.ai-memory/test-inventory.json`, and records test execution timings.

##### 2. Files to Modify
- `03-ai-scripts/06-cicd-local-runner.py`:
  - Define `TEST_INVENTORY_PATH = Path(".ai-memory/test-inventory.json")`
...
- `03-ai-scripts/06-cicd-local-runner.py`:
  ...
  - In Section 5 (Go Smart Incremental Tests): run only changed/affected tests via the worker group, parse individual test results and durations, and update `.ai-memory/test-inventory.json`.
...
##### Files to Touch
- `.ai-memory/plans/completed/85-parallel-cpu-checkers-and-live-progress-engine.md`
...
2. **Git Commit History Changed-Files Extractor (`03-ai-scripts/27-git-changed-files.py`)**:
   - Standalone CLI utility with `--commits <N>` / `-n <N>` (default: 20), `--output-dir` (default: `.ai-memory/temp/`), `--format` (`all`, `json`, `yaml`, `txt`).
   - Exports:
     - `.ai-memory/temp/git-changed-files.json`
     - `.ai-memory/temp/git-changed-files.yaml`
     - `.ai-memory/temp/git-changed-files.txt`
...
3. **High-Speed Chunked Relative Paths Checker (`linter-scripts/check-relative-paths.py`)**:
   ...
   - Support `--changed-only` / `-c` and `--recent [N]` reading from `.ai-memory/temp/git-changed-files.json`.
...
4. **Nested Ifs & Enum/Boolean Checkers Upgrades**:
   ...
   - Support `--changed-only` reading from `.ai-memory/temp/git-changed-files.json`.
...
##### Success Criteria
- [ ] `03-ai-scripts/27-git-changed-files.py` extracts changed files from last 20 commits into JSON, YAML, and TXT in `.ai-memory/temp/` in < 0.2s.
...
##### Objective
Create standalone CLI script `03-ai-scripts/27-git-changed-files.py` to extract changed files from the last $N$ commits into `.ai-memory/temp/` in JSON, YAML, and TXT formats with dictionary deduplication.

##### Proposed Changes
1. CLI Arguments:
   - `--commits`, `-n`: Number of commits to inspect (default: 20).
   - `--output-dir`, `-o`: Target directory (default: `.ai-memory/temp/`).
...
3. Multi-Format Output:
   - `.ai-memory/temp/git-changed-files.json`
   - `.ai-memory/temp/git-changed-files.yaml` (pure python yaml serializer without external deps)
   - `.ai-memory/temp/git-changed-files.txt` (one path per line for easy shell iteration)
4. Speed:
   - Must complete in < 0.2 seconds.

##### Acceptance Criteria
- [ ] Script runs via `python 03-ai-scripts/27-git-changed-files.py` with exit code 0.
- [ ] All 3 files generated in `.ai-memory/temp/`.
...
2. Dynamic Discovery & Changed-Files Resolution:
   - If `--changed-only` is passed, check if `.ai-memory/temp/git-changed-files.json` exists.
...
1. `linter-scripts/check-nested-ifs.py`:
   ...
   - Add `--changed-only` support reading from `.ai-memory/temp/git-changed-files.json`.
...
1. `03-ai-scripts/06-cicd-local-runner.py`:
   ...
   - When active, ensure `.ai-memory/temp/git-changed-files.json` is generated by invoking `03-ai-scripts/27-git-changed-files.py`.
...
- [`.ai-memory/memory/learned/03-parallel-cicd-runner-and-log-filtering.md`](.ai-memory/memory/learned/03-parallel-cicd-runner-and-log-filtering.md)
- [`.ai-memory/memory/issues/2026-09-02-cicd-runner-hang-macos.md`](.ai-memory/memory/issues/2026-09-02-cicd-runner-hang-macos.md)
