...
- [x] [01-signature-scanner.md](.ai-memory/plans/subtasks/24-function-signatures/01-signature-scanner.md) — Create `check-function-formatting.py` and `check-function-signatures.py`.
- [x] [02-result-envelope-methods.md](.ai-memory/plans/subtasks/24-function-signatures/02-result-envelope-methods.md) — Implement typed `Result[T]` methods (`IsSuccess`, `IsFailed`, `Unwrap`).
- [x] [03-predicate-prefixes.md](.ai-memory/plans/subtasks/24-function-signatures/03-predicate-prefixes.md) — Verify `is`/`has`/`can`/`should` prefixes across boolean functions.
- [x] [04-verification.md](.ai-memory/plans/subtasks/24-function-signatures/04-verification.md) — Verify all signature and error quality gates exit with code 0.
...
- [x] [01-result-envelope-and-guards.md](.ai-memory/plans/subtasks/25-typescript-types/01-result-envelope-and-guards.md) — Implement strongly-typed `Result<T>` envelope and exhaustive pattern matching helpers.
- [x] [02-type-safety-audit.md](.ai-memory/plans/subtasks/25-typescript-types/02-type-safety-audit.md) — Audit codebase for `any` types and verify zero unsafe type assertions.
- [x] [03-verification.md](.ai-memory/plans/subtasks/25-typescript-types/03-verification.md) — Verify `tsc --noEmit`, vitest test suites, and all quality gates.
...
- [x] [01-enum-scanner.md](.ai-memory/plans/subtasks/26-enums-and-traits/01-enum-scanner.md) — Create `check-enum-guidelines.py` and verify enum suffixes across languages.
- [x] [02-type-suffix-enforcement.md](.ai-memory/plans/subtasks/26-enums-and-traits/02-type-suffix-enforcement.md) — Enforce `*Type` naming across Go, TypeScript, PHP, Rust, and Python enums.
- [x] [03-pattern-matching-helpers.md](.ai-memory/plans/subtasks/26-enums-and-traits/03-pattern-matching-helpers.md) — Implement exhaustive pattern matching and helper methods (`IsValid`, `IsTerminal`, `assertNever`).
- [x] [04-verification.md](.ai-memory/plans/subtasks/26-enums-and-traits/04-verification.md) — Run all enum linters and quality gates.
...
5. `05-linter-and-ci-verification.md` — Registered parameter scanner in `03-ai-scripts/` and verified all 23 CI gates exit 0. [COMPLETED]
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/31-enums/01-enum-type-suffix.md`)**:
   Refactor Go enum types (`MatchKind`, `OutputMode`, `CompressionMode`, `OutputFormat`) to `*Type` definitions.
2. **Subtask 02 (`.ai-memory/plans/subtasks/31-enums/02-constants-and-runes-verification.md`)**:
   Verify zero raw rune casts (`rune(10)`), zero magic string delimiters, and enforce dedicated constant packages.
3. **Subtask 03 (`.ai-memory/plans/subtasks/31-enums/03-ci-linter-verification.md`)**:
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/32-react-frontend/01-custom-hook-signatures.md`)**:
   Verify that all custom hooks in `src/hooks/` return named property objects rather than tuple arrays.
2. **Subtask 02 (`.ai-memory/plans/subtasks/32-react-frontend/02-immutable-state-and-useeffect.md`)**:
   Verify absence of direct state mutations and redundant `useEffect` derived state syncing.
3. **Subtask 03 (`.ai-memory/plans/subtasks/32-react-frontend/03-ci-frontend-build-verification.md`)**:
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/37-function-signatures/01-multi-line-formatting-and-signatures.md`)**:
   Format function definitions and invocations with >2 parameters/arguments to one per line.
2. **Subtask 02 (`.ai-memory/plans/subtasks/37-function-signatures/02-result-envelope-and-apperror.md`)**:
   Verify `Result[T]` envelope and `AppError` predicate methods.
3. **Subtask 03 (`.ai-memory/plans/subtasks/37-function-signatures/03-ci-runner-verification.md`)**:
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/38-typescript/01-strict-types-and-discriminated-unions.md`)**:
   Verify total absence of `any`, validate discriminated unions, and check `as const` enums.
2. **Subtask 02 (`.ai-memory/plans/subtasks/38-typescript/02-tsc-and-build-verification.md`)**:
   Execute `npx tsc --noEmit` and `npm run build` to guarantee type safety and bundling.
3. **Subtask 03 (`.ai-memory/plans/subtasks/38-typescript/03-ci-runner-verification.md`)**:
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/40-enums/01-enum-type-suffix-and-traits.md`)**:
   Verify `*Type` suffixes, traits, and exhaustive match handling.
2. **Subtask 02 (`.ai-memory/plans/subtasks/40-enums/02-linters-and-ci-runner.md`)**:
...
1. **Subtask 01 (`.ai-memory/plans/subtasks/42-argument-reduction/01-parameter-struct-reduction.md`)**:
   Enforce value-based parameter structs (`*Params`) for functions with >2–3 parameters and affirmative boolean field prefixes.
2. **Subtask 02 (`.ai-memory/plans/subtasks/42-argument-reduction/02-apperror-and-result-returns.md`)**:
   Mandate `*apperror.AppError` returns for side-effect operations and `Result[T]` for data-producing functions.
3. **Subtask 03 (`.ai-memory/plans/subtasks/42-argument-reduction/03-ci-runner-verification.md`)**:
   Execute full CI quality gates via `python 03-ai-scripts/06-cicd-local-runner.py`.
