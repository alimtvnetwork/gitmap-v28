...
1. **Directory Isolation**:
   - Test temporary root is strictly `.ai-memory/temp/` (NOT global system `/tmp` or root `temp/`).
   - Failure output is isolated strictly to `.ai-memory/temp/failures/<test_id>.log`.
   - Passing tests are 100% silent in both console output and the filesystem (zero logs created).
2. **Centralized Test Inventory (`.ai-memory/test-inventory.json`)**:
...
4. **Dynamic ETA Sleep Protocol**:
   - `06-cicd-local-runner.py` writes live telemetry and ETA estimates to `.ai-memory/temp/runner-eta.json` calculated from past runs.
