...
3. **Zero Test Inventory Timing**:
   - `.ai-memory/test-inventory.json` catalogs 2,498 tests, but all tests currently record `duration_sec: 0.0` and `last_status: "never_run"`.
...
2. **Inventory Tagging**:
   - Update `.ai-memory/test-inventory.json` with duration estimates and separate category tags (`tier: "unit"` vs `tier: "heavy"`).
...
1. `01-heavy-test-analysis-and-inventory-estimation.md`:
   - Estimate durations for all 2,498 tests in `.ai-memory/test-inventory.json`.
...
4. `04-test-inventory-sync-and-verification.md`:
   - Re-scan and generate updated `.ai-memory/test-inventory.json` with new package paths.
...
4. Updated .ai-memory/test-inventory.json: 121 heavy tests (259.53s) vs 2724 unit tests (13.62s).
