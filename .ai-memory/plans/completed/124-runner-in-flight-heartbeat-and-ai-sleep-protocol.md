...
> 2. AI agents and runner prompts updated with a mandatory **1 minute (60 seconds) sleep/wait protocol** each time, or dynamically sleeping for the remaining duration read from `.ai-memory/temp/runner-eta.json` (or based on previous known approximate total delay) instead of busy-polling.
> 3. Synchronized across `01-prompts/`, `.agents/skills/`, `.ai-memory/memory/`, and connected workspace `coding-guidelines`.
...
1. **Terminal Noise from Rapid Heartbeats**:
   ...
   - The user provided terminal artifact `.ai-memory/assets/terminals/04-runner-in-flight-interval.png` showing rapid emissions.
...
2. **AI Agent Busy-Polling**:
   ...
   - Solution: Prompts and skills now strictly mandate sleeping for **1 minute (60 seconds)** per iteration, or dynamically sleeping for `remaining_eta_sec` from `.ai-memory/temp/runner-eta.json` / previous total approximate delay.
...
- Updated Runner In-Flight ETA Wait Protocol in `gitmap`:
  ...
  - `.ai-memory/memory/learned/14-smart-test-runner-and-temp-isolation.md` (Section D)
...
- Image asset `.ai-memory/assets/terminals/04-runner-in-flight-interval.png` tracked and persisted.
