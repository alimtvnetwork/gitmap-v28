...
> Started in response to the user's directive regarding test directory isolation (.ai-memory/temp/ vs root .tmp/), dedicated failure output folder (.ai-memory/temp/failures/), silent passing tests, non-empty relative path mapping in .ai-memory/test-inventory.json, configurable slow test thresholds, dual worker queue architecture (slow: 4 workers x 2 tests; fast: 4 workers x 4 tests in chunks of 100), in-flight ETA synchronization (.ai-memory/temp/runner-eta.json), and AI wait/sleep protocol across both `gitmap` and `coding-guidelines` workspaces.
> Completed autonomously across 6 execution steps/subtasks.

## 1. Problem Diagnosis & Architectural Goal
1. **Unwanted `.tmp/` Directory Pollution**: Tests and runners previously defaulted `TMP_CACHE_DIR` to `.tmp/` at the repository root, polluting git status and disk. All temporary files, runner caches, and test artifacts are now strictly restricted to `.ai-memory/temp/`.
2. **Failure Logging & Silent Passing Tests**: Passing tests now produce zero filesystem artifacts and remain completely silent in output logs. Only failing tests/quality gates write detailed error logs to `.ai-memory/temp/failures/{test_or_job}.log`.
3. **Relative Path Code-to-Test Mapping**: The centralized inventory (`.ai-memory/test-inventory.json`) guarantees non-empty, repository-relative forward-slash paths for both `code_file` (`target_file`) and `test_file` (0 empty target files across 3,534 tests, 0 absolute paths). Tests execute if and only if their code or test file has changed.
...
1. **Rule T1 (.ai-memory/temp Isolation)**: Never write temporary test directories or cache files to root `.tmp/` or OS temp. All temp data resides in `.ai-memory/temp/`.
2. **Rule T2 (Zero-Trace Passing Tests)**: Passing tests MUST NEVER write files to disk or emit verbose test logs. Only failed tests write logs to `.ai-memory/temp/failures/{test_id}.log`.
3. **Rule T3 (Strict Relative Git Paths)**: All file paths in `.ai-memory/test-inventory.json` must be strictly relative forward-slash paths (e.g. `gitmap/cmdmacro/helpers.go`).
...
5. **Rule T5 (AI ETA Sleep Discipline)**: When checking running background commands, if the task is still running, AI must read remaining ETA from `.ai-memory/temp/runner-eta.json` and sleep until completion rather than poll repeatedly.

## 3. Subtask Consolidation & Deliverables

### Subtask 1: Temp & Failure Directory Isolation (.ai-memory/temp/failures/) (Completed)
- In `03-ai-scripts/06-cicd-local-runner.py`:
  - Migrated `TMP_CACHE_DIR` to `REPO_ROOT / ".ai-memory" / "temp"`.
  - Created `FAILURES_DIR = TMP_CACHE_DIR / "failures"`.
  - Configured `GOTMPDIR`, `TMPDIR`, `TEMP`, `TMP` pointing to `.ai-memory/temp/`.
  - In `run_package_tests_worker` and `run_smart_go_tests`, passing tests produce zero output and write zero files. Failed tests write logs to `.ai-memory/temp/failures/{safe_tid}.log`.
...
- Replaced static 5.0s estimate in `calculate_total_eta` with dynamic duration aggregation from `.ai-memory/test-inventory.json`.
- Clamped remaining ETA to `>= 1s` during active jobs to eliminate premature 0s display.
- Persisted live progress to `.ai-memory/temp/runner-eta.json` with status, elapsed, total estimated, remaining ETA, and total/completed counts.
...
- Synchronized `03-ai-scripts/33-test-inventory-generator.py` and `03-ai-scripts/06-cicd-local-runner.py` with failure isolation, `.ai-memory/temp/` isolation, and ETA updates to `D:\wp-work\riseup-asia\coding-guidelines\`.
...
- Recorded 10 modified files to `.ai-memory/temp/recent-file-changes.json` under lock.
