...
5. **Rule 5 (Cache Tracking)**: All modified files recorded in `.ai-memory/temp/recent-file-changes.json` under atomic lock via `python 03-ai-scripts/33-test-inventory-generator.py --record`.
...
- `.ai-memory/strictly-avoid.md`: Added total ban on un-namespaced OS temp writes and un-cleaned builds.
- `.ai-memory/coding-guidelines.md`: Added Temp Directory & Storage Hygiene section.
- `02-spec/02-coding-guidelines/01-cross-language/29-no-generated-artifacts.md`: Added Storage Hygiene, Temp Directory Isolation & Pre-Build Cleanup section.
