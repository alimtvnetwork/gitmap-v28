...
1. Verify and ensure `03-ai-scripts/06-cicd-local-runner.py`:
   - Failure folder isolation: `.ai-memory/temp/failures/` is the only directory where failed test logs are written (`<test_id>.log`).
   ...
   - Live ETA calculation and telemetry written to `.ai-memory/temp/runner-eta.json`:
...
- [x] Test inventory `.ai-memory/test-inventory.json` uses strictly relative paths.
