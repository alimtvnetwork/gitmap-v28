...
1. Inspect and synchronize `03-ai-scripts/06-cicd-local-runner.py` in `D:\wp-work\riseup-asia\coding-guidelines`:
   ...
   - Failure folder isolation: `.ai-memory/temp/failures/` is the sole sink for failing test logs (`<test_id>.log`).
   - Passing tests: 100% silent in terminal output and filesystem.
   - Dynamic ETA calculation and telemetry written to `.ai-memory/temp/runner-eta.json` for AI agent sleep protocol.
