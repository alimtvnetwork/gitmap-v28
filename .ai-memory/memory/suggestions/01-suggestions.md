...
- ✅ Create `02-spec/03-general/` with reusable design guidelines
...
- ✅ Made all `02-spec/03-general/` files fully generic
...
- ✅ Add `--dry-run` flag to `clone-next` for previewing actions without executing (already implemented v3.132.0+ — see cli/cmd/clonenextdryrun.go)
...
- ✅ Add `gitmap uninstall <tool>` command — already implemented (cli/cmd/uninstall.go: choco/winget/apt/brew/snap, --dry-run/--force/--purge, DB cleanup, self-uninstall delegation when no tool name); now also documented in src/data/commands.ts + DocsSidebar.tsx
- ✅ Enhanced `install --list` grouped by category with installed status (cli/cmd/installlist.go: groups by ToolCategoryCore/Database, status from InstalledTool DB + PATH probe fallback, ●/○ glyphs + legend)
- ✅ Unit tests for task, env, and install commands — install ✅ (cli/cmd/install_unit_test.go), env ✅ (cli/cmd/env_unit_test.go + envplatform_windows_test.go), task ✅ (cli/cmd/task_unit_test.go: isGitignoreComment table, parseGitignoreLines strip+empty, matchGlob basename + bad-pattern, matchesPattern dir-vs-file matrix, isIgnored short-circuit + any-match-wins). Note: pendingtaskhelper_test.go already covered buildCommandArgs / findDuplicate / pending-task lifecycle.
...
- ✅ Dedupe `captureStderr` test helper in `cli/cmd/` — extracted to `capturestderr_testhelper_test.go` (goroutine-drain variant, deadlock-safe for >64KiB payloads); removed duplicate definitions from `clonepmsync_debugpaths_test.go` and `scanworkersalias_test.go`, pruned now-unused `io`/`bytes`/`os` imports. Closes the duplicate-symbol risk flagged in `.ai-memory/question-and-ambiguity/02-cmd-test-helper-duplicates.md` (the `collectObjectKeys` / `equalStringSlices` pair from that ticket was already cleaned up — only `captureStderr` remained).
- ✅ Dedupe `withFakeLaunchAgentsDir` in `cli/startup/` — extracted to `launchagents_testhelper_test.go`; removed duplicate definitions from `add_darwin_test.go` and `plist_test.go` (both had identical bodies, no build tags, real Go redeclaration). `runtime` import retained in both call-site files (still used by other `runtime.GOOS` guards). Closes the second pre-existing duplicate flagged in `.ai-memory/question-and-ambiguity/04-startup-lifecycle-integration-tests.md`.
- ✅ `startup/win*.go` linux build leak — verified RESOLVED (no action required). Audit: ran `go build ./...` and `go test -count=0 ./...` from `/dev-server/gitmap` on Linux (Go 1.25.7 via nix). Both pass clean. Root-cause analysis of why ticket 08's concern is no longer reproducible: `winbackend.go` and `winshortcut.go` are intentionally untagged (cross-OS dispatch with `runtime.GOOS != "windows"` runtime guards), and every Windows-only symbol they reference (`addWindowsRegistry{,HKLM}`, `removeWindowsRegistry{,HKLM}`, `listWindowsRegistry{,HKLM}`, `trackingSubkeyExists`, `writeTrackingSubkey`, `deleteTrackingSubkey`) has a non-Windows stub in `cli/startup/winregistry_other.go` (`//go:build !windows`). The remaining startup-folder helpers (`addWindowsStartupFolder`, etc.) live in the untagged `winshortcut.go` itself, so they're trivially available cross-platform. Closes the follow-up flagged at the bottom of `.ai-memory/question-and-ambiguity/08-jsonl-format-design.md`.
- ✅ JSON schema migration: `gitmap list-releases --json` (high priority, top of `02-spec/08-json-schemas/_TODO.md`) — migrated both surfaces (per-repo + `--all-repos` joined view) from `json.MarshalIndent` to `cli/stablejson`. New file `cli/cmd/listreleasesrender.go` exposes `encodeListReleasesJSON` + `encodeListReleasesAllReposJSON` with explicit field-order constants. Two schemas published: `02-spec/08-json-schemas/list-releases.schema.json` (per-repo, lowerCamel keys mirror `model.ReleaseRecord` json tags) + `02-spec/08-json-schemas/list-releases-all-repos.schema.json` (PascalCase keys preserve legacy `store.ReleaseAcrossRepos` MarshalIndent surface — `ReleaseID`/`RepoSlug`/etc, no json tags). Pinned by `cli/cmd/listreleases_jsonschema_contract_test.go` — 9 tests covering top-level shape, required keys, propertyOrder == emitted order, empty-as-`[]`, and **byte-compat with legacy `json.MarshalIndent` output for both shapes** (the headline migration guarantee — downstream golden-file consumers see zero diff). README + _TODO.md updated.
