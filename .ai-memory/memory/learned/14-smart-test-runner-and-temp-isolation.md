...
### A. Temp & Failure Directory Isolation (.ai-memory/temp/failures/)
- All temporary directories, runner caches, and test run environments are restricted strictly to `.ai-memory/temp/`.
- Creating or writing to `.tmp/` at the repository root is strictly prohibited.
- `GOTMPDIR`, `TMPDIR`, `TEMP`, and `TMP` environment variables are mapped to `.ai-memory/temp/`.
- Dedicated failure directory: `.ai-memory/temp/failures/` is established. Only failing tests/gates write `<test-or-job>.log` there.
...
- Dynamic ETA evaluation: computes aggregate duration of dirty tests from `.ai-memory/test-inventory.json` rather than static 5.0s constants.
- ETA clamping: remaining ETA is clamped to `>= 1s` while jobs are in flight, preventing premature 0s display.
- Live progress persistence: status, elapsed, total estimated, remaining ETA, and total/completed counts streamed to `.ai-memory/temp/runner-eta.json`.
- AI Sleep Discipline (1 Minute / Dynamic ETA): When an AI agent launches or checks an active background runner, the agent MUST sleep/wait for **1 minute (60 seconds) each time**, or dynamically sleep for the remaining ETA duration read from `.ai-memory/temp/runner-eta.json` (or based on previous total approximate delay) instead of busy-polling.
