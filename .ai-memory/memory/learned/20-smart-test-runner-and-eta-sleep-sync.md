...
2. **Directory & Failure Isolation**:
   - Temporary artifacts, caches, and ETA telemetry reside strictly in `.ai-memory/temp/`. Root-level `temp/` is prohibited.
   - Failures are captured individually in `.ai-memory/temp/failures/<test-id>.log`.
   - Passing tests are 100% silent in terminal output and the filesystem (no logs created).
3. **Relative Path Manifests**:
   - `.ai-memory/test-inventory.json` stores all paths (`target_file` and `test_file`) as repo-relative forward-slash paths (e.g. `cli/cmd/root.go`, `cli/cmd/root_test.go`).
...
4. **Dynamic ETA Sleep Protocol**:
   - Runner writes progress and remaining duration estimates to `.ai-memory/temp/runner-eta.json`.
