...
5. **Inline cache writes into the last validation job** — a separate `mark-success` job can be canceled by `cancel-in-progress` after all validation passes, leaving the SHA uncached. Inlining the cache save as the final step of `test-summary` prevents this.
