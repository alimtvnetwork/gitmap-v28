...
4. **`v3.12.x` is a minor line.** Per project policy (`.ai-memory/user-preferences` line 8: *"Code changes must bump at least minor version"*) and standard semver, we cannot drop backward compat in a patch or minor release. The earliest defensible removal point is **v4.0.0**.
...
- Spec: this audit should eventually be summarized into `02-spec/01-app/` as part of the v4.0 breaking-change matrix when that doc is created.
