...
A sub-package upgrade or dependency addition pulled in bleeding-edge versions of `golang.org/x/*` packages (`v0.55.0`, `v0.47.0`, `v0.41.0`) which have minimum version requirements set to `go 1.25.0`, causing `cli/go.mod` to declare `go 1.25.0`.
...
- `cli/go.mod` line 3 declared `go 1.25.0` with `golang.org/x/crypto v0.55.0`, `golang.org/x/sys v0.47.0`, and `golang.org/x/text v0.41.0`.
- Incompatibility between Go 1.25 dependencies and the project-pinned `golangci-lint v1.64.8` Go 1.24 toolchain.

## 4. Code Fix

- Reverted `cli/go.mod` to `go 1.24.13` and downgraded `golang.org/x/*` dependencies to stable Go 1.24 releases (`crypto v0.36.0`, `sys v0.41.0`, `term v0.40.0`, `text v0.23.0`, `tools v0.31.0`).
...
- Verified all packages pass `go test ./...` and `03-ai-scripts/02-cicd-local-runner.py` with 0 failures.
