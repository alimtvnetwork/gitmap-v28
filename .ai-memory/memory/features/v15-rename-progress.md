...
PascalCase + **singular** table names + `{TableName}Id` primary keys + FKs match referenced PK name + `IsX` boolean prefix + abbreviations as words. Per https://github.com/alimtvnetwork/coding-guidelines-v15/blob/main/02-spec/04-database-conventions/01-naming-conventions.md.
...
- **Regenerated `02-spec/01-app/gitmap-database-erd.mmd`** to v15 names: every table singular, every PK is `{TableName}Id`, FKs match (`GoRunnableFile.GoProjectMetadataId`, `CsharpProjectFile.CsharpProjectMetadataId`, etc.), `Release` has `IsDraft`/`IsPreRelease`/`IsLatest`, `SshKey`/`Csharp*` reflect abbreviation rule.
- **Updated `02-spec/12-consolidated-guidelines/11-database.md`** Schema Conventions table — singular names, `{TableName}Id` PKs, IsX boolean prefix row, reserved-word quoting row, abbreviation rules row, link to upstream v15 spec.
...
- Phase 1.6: regenerate both ERDs, update 02-spec/12-consolidated-guidelines/11-database.md, CHANGELOG entry, mem://index core update, version bump to v3.4.0 (or higher).
