---
name: commit-in
description: gitmap commit-in / cin replays commits from N source repos into one destination repo, dedupes by source SHA, replicates author+committer dates, profiles in .gitmap/commit-in/profiles/. Spec at 02-spec/03-commit-in/.
type: feature
---
...
- Spec: `02-spec/03-commit-in/` (7 iterations: overview, CLI surface,
  pipeline, DB schema + ERD, profiles + JSON, message + function-intel,
  acceptance + edge cases).
- Plan: `.ai-memory/plan.md` § "commit-in / cin — 2026-05-06" — 7 phased
  implementation steps, gated on the user typing `next`.
- Internal note: `.ai-memory/memory/issues/2026-05-06-commit-in-spec.md`
