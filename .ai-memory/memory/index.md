...
NO-QUESTIONS MODE ACTIVE (40-task budget): never ask user clarifying questions. Log every ambiguity to `.ai-memory/question-and-ambiguity/xx-brief-title.md` (sequential numbering, update `00-index.md` table). Each entry: original task + spec ref, ambiguity, all options w/ pros/cons, recommendation, decision taken. Proceed with best inference. Resume questions only when user says "ask question".
...
Consumer-facing JSON outputs use `cli/stablejson` (key-by-key, no struct reflection) so field order cannot drift across Go versions or encoding/json/v2.
...
HD HOSTED FALLBACK (v6.0.x+): `gitmap hd` MUST NOT `os.Exit(1)` when `docs-site/` is missing, `docs-site.zip` download 404s, or extract fails. Instead call `openHostedDocsFallback()` which prints `MsgHDHostedFallback` + opens `constants.DocsURL` (`https://gitmap.dev/docs`). Pinned by `TestHostedDocsFallbackContract` (`cli/constants/constants_helpdashboard_test.go`) for the constants and `TestOpenHostedDocsFallbackPrintsURL` + `TestOpenURLNonFatalOnMissingLauncher` (`cli/cmd/helpdashboard_fallback_test.go`) for the runtime invariant (URL printed to stderr before browser launch; missing `xdg-open`/`open`/`start` is non-fatal). See 02-spec/02-app-issues/34-hd-hosted-docs-fallback.md.
...
Cross-platform install/update reference (v3.100.0+): canonical matrix at `02-spec/01-app/108-cross-platform-install-update.md`, mirrored on `/install-gitmap` page and linked from README top.
...
- [What to Read (AI onboarding map)](.ai-memory/memory/project/what-to-read.md) — Read-order, folder map, JSON output contract triangle (schema ↔ model ↔ encoder ↔ test ↔ fixture), and step-by-step recipe for adding a new JSON output. **Start here on a fresh session.**
...
- [Cross-Platform Install/Update](mem://features/cross-platform-install-update) — Canonical Win/macOS/Linux install · update · uninstall · verify matrix at 02-spec/01-app/108-cross-platform-install-update.md, mirrored at /install-gitmap (v3.100.0)
...
- [Release gitmap-only body + snapshots](mem://features/release-gitmap-only-body-and-snapshots) — non-gitmap repos get tag-only releases (empty body, no release-version.{ps1,sh} snapshots); gated by ShouldPrintInstallHint (v5.16.0+, 02-spec/02-app-issues/27)
...
- [Stable JSON Encoding](mem://features/stable-json-encoding) — `cli/stablejson` package: key-by-key encoding, byte-compat with json.Encoder, used by `startup-list --format=json` (v3.152.0)
...
- [Transaction Revert](mem://features/transaction-revert) — Every FS-mutating gitmap op writes a Transaction row + per-file backups under .gitmap/txn/<id>/data/<repo>/<git-sha>/files/, revertable via the existing `gitmap revert` command (--list-txn / --txn / --last-txn / --prune-txn / --force). Cap 50, fix-repo --replace exempt. Spec: 02-spec/04-generic-cli/28-transaction-revert.md
- [Commit-In](mem://features/commit-in) — gitmap commit-in / cin: replay commits from N input repos into one source repo, dedupe by SourceSha, replicate dual dates, profile-driven message+exclusion+function-intel rules, auto-init source (URL/repo/non-repo folder/missing). SPEC ONLY at 02-spec/03-commit-in/, 7-phase plan in plan.md.
- [Strictly-Prohibited](mem://constraints/strictly-prohibited) — Append-only numbered registry of forbidden actions; mirrors 02-spec/03-general/10-strictly-prohibited.md. Entry #3 forbids file content/hash in commit-in tables; #4 forbids rewriting source-repo history.
...
- [02-gitmap-installer.md](.ai-memory/memory/learned/02-gitmap-installer.md) - Learned patterns for Gitmap installer system
- [03-project-architecture-and-skills.md](.ai-memory/memory/learned/03-project-architecture-and-skills.md) - Synthesis of repository architecture, guidelines, and codified skills
- [2026-08-29-coding-guideline-violations.md](.ai-memory/memory/issues/2026-08-29-coding-guideline-violations.md) - RCA on coding guideline violations and 150-step audit
- [2026-08-29-go-125-dependency-linter-break.md](.ai-memory/memory/issues/2026-08-29-go-125-dependency-linter-break.md) - RCA on Go 1.25 dependency version bump causing CI linter break
- [2026-08-29-go-125-gofmt-and-goimports-drift.md](.ai-memory/memory/issues/2026-08-29-go-125-gofmt-and-goimports-drift.md) - RCA on Go 1.25 gofmt and goimports drift across all repository packages
- [2026-08-29-installctx-e2e-with-explain-type-mismatch.md](.ai-memory/memory/issues/2026-08-29-installctx-e2e-with-explain-type-mismatch.md) - RCA on installctx E2E withExplain type signature mismatch under Linux/Darwin
- [2026-08-29-legacy-refs-bare-err-and-lint-keyerror.md](.ai-memory/memory/issues/2026-08-29-legacy-refs-bare-err-and-lint-keyerror.md) - RCA on legacy refs whitelist, bare stderr prints in cmd, and lint script KeyError
- [2026-08-29-lfs-zip-drift-changelog-sync-and-jq-diff-argjson.md](.ai-memory/memory/issues/2026-08-29-lfs-zip-drift-changelog-sync-and-jq-diff-argjson.md) - RCA on Git LFS zip drift, changelog version sync, and jq diff argjson
- [2026-08-29-smoke-installer-var-version.md](.ai-memory/memory/issues/2026-08-29-smoke-installer-var-version.md) - RCA on smoke-installer.sh regex failing to parse var Version in constants.go
- [2026-08-29-docs-site-usetheme-syntax-error.md](.ai-memory/memory/issues/2026-08-29-docs-site-usetheme-syntax-error.md) - RCA on check!res typo in useTheme.ts breaking docs-site build
- [2026-08-29-root-cli-panic-on-zero-args.md](.ai-memory/memory/issues/2026-08-29-root-cli-panic-on-zero-args.md) - RCA on panic("fatal error") in cmd/root.go and across command files
- [04-full-codebase-architecture-and-skill-expansion.md](.ai-memory/memory/learned/04-full-codebase-architecture-and-skill-expansion.md) - Full codebase architecture load, memory synthesis, and frontend/cluster skill expansion
