...
4. **Query Wrappers**: Go, Python, PHP, and TS must utilize a `queryWrapper` that automatically intercepts and logs query failures according to `.ai-memory/error-manage/guidelines.md`. The return object must contain explicit `isSuccess` and `isFailure` booleans.
