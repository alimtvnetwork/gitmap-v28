// Package ecosystemgroup manages group persistence for AGY, VSCode, Chrome, and GitHub Desktop.
...
// GetStorePath returns the JSON storage path for an ecosystem.
func GetStorePath(ecosystem string) (string, *apperror.AppError) {
...
// LoadStore loads the group store from disk.
func LoadStore(ecosystem string) (*EcosystemGroupStore, *apperror.AppError) {
	filePath, pathErr := GetStorePath(ecosystem)
	if pathErr != nil {
		return nil, pathErr
	}
...
// SaveStore writes the group store to disk.
func SaveStore(store *EcosystemGroupStore) *apperror.AppError {
	filePath, pathErr := GetStorePath(store.Ecosystem)
	if pathErr != nil {
		return pathErr
	}
...
// NewGroup creates a new EcosystemGroup instance.
func NewGroup(name string, desc string, targets []string) EcosystemGroup {
