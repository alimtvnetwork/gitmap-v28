...
import "github.com/alimtvnetwork/gitmap-v28/gitmap/cmd"
...
func TestApplyTransportFlag_E2E(t *testing.T) {
	...
	t.Run("https_to_ssh_persists", func(t *testing.T) {
		changed, oldURL, newURL, err := cmd.ApplyTransportFlag(work, true, false)
		if err != nil {
	...
	t.Run("ssh_to_https_persists", func(t *testing.T) {
		changed, _, _, err := cmd.ApplyTransportFlag(work, false, true)
		if err != nil {
			t.Fatalf("ApplyTransportFlag: %v", err)
		}
	...
	t.Run("ssh_wins_when_both_set", func(t *testing.T) {
		changed, _, newURL, err := cmd.ApplyTransportFlag(work, true, true)
		if err != nil {
			t.Fatalf("ApplyTransportFlag: %v", err)
		}
	...
	t.Run("unrecognized_url_fails_open", func(t *testing.T) {
		...
		changed, _, _, err := cmd.ApplyTransportFlag(work, true, false)
		if err != nil {
