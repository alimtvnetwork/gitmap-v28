...
func injectVersionBake(body, embeddedName, bakeLine string) string {
	...
	isPS1 := strings.HasSuffix(embeddedName, ".ps1")
	ps1Marker := "$ErrorActionPreference"
	idx := strings.Index(body, ps1Marker)
	...
	bashMarker := "set -euo pipefail"
	idxBash := strings.Index(body, bashMarker)
	if idxBash < 0 {
