...
Generated: 2026-07-23 14:06 UTC. Companion to `.ai-memory/memory/reports/20260723-rejog-reliability.md`.
...
- **spec-hygiene**: everything under `02-spec/` and `.ai-memory/`.
...
- **Objective**: Make `.ai-memory/overview.md` and `.ai-memory/memory/index.md` state the same version as `cli/constants/constants.go`.
- **Dependencies**: none.
- **Expected outputs**: edits to `.ai-memory/overview.md` and `.ai-memory/memory/index.md`. A one-line note naming `cli/constants/constants.go` as the source of truth.
- **Acceptance criteria**:
  - Grep for the old versions (v3.1.0, v5.9.0) returns 0 matches in `.ai-memory/`.
  - Both files display the version currently in `constants.go`.
  - Files still parse as valid markdown.

### P0-2: Deduplicate numeric prefixes in 02-spec/01-app and 02-spec/02-app-issues

- **Project**: spec-hygiene
- **Objective**: Every spec file has a unique numeric prefix, and a README maps prefixes to topics.
- **Dependencies**: none.
- **Expected outputs**:
  - `02-spec/01-app/README.md` and `02-spec/02-app-issues/README.md` mapping prefix to file.
...
- **Acceptance criteria**:
  - `ls 02-spec/01-app | awk -F- '{print $1}' | sort | uniq -d` returns empty.
  - Same for `02-spec/02-app-issues`.
...
- **Dependencies**: none.
- **Expected outputs**: edits to `02-spec/05-coding-guidelines/01-*.md`, `02-spec/05-coding-guidelines/02-go-code-style.md`, and `02-spec/12-consolidated-guidelines/02-go-code-style.md`.
...
### P1-2: Resolve open SSH bug (`.ai-memory/issues/01-ssh-repo-cloned-as-https.md`)
...
- **Acceptance criteria**: automated test passes; issue file moved to `.ai-memory/solved-issues/`.

### P1-3: `commit-in` command

- **Project**: gitmap-cli
- **Objective**: Implement per `02-spec/03-commit-in/`.
...
- **Acceptance criteria**: spec `01-overview-and-glossary` invariants hold; profile schema matches `02-spec/03-commit-in/05-profiles-and-json-shape.md`.
...
- **Project**: gitmap-cli
- **Objective**: Close gap flagged in `.ai-memory/pending-issues/`.
- **Dependencies**: none.
...
### P2-2: Compress `01-prompts/03-*` through `20-next-task.md`

- **Project**: spec-hygiene
- **Objective**: One canonical `next-task.md`; older per-invocation copies moved to `01-prompts/archive/`.
- **Dependencies**: none.
...
### P2-3: Deduplicate `.ai-memory/02-spec/commands/` vs `02-spec/01-app/`

- **Project**: spec-hygiene
- **Objective**: Single source of truth.
- **Dependencies**: P0-2.
- **Expected outputs**: `.ai-memory/02-spec/commands/README.md` redirecting to root `02-spec/01-app/`.
...
- **Dependencies**: P0-2.
- **Expected outputs**: `02-spec/01-app/CONTRACT-TESTS.md`.

### P2-5: Docs-site content parity with shipped features

- **Project**: docs-site
- **Objective**: Ensure pages under `src/pages/` cover every command listed in `02-spec/01-app/` up to the current version.
