...
>
> - All `*At` timestamp columns MUST be `INTEGER` (epoch seconds, UTC) per [`02-spec/04-database-conventions/03-naming-conventions.md`](../04-database-conventions/02-naming-conventions.md) **Rule 7.1 v2**. TEXT/ISO-8601 storage is forbidden for new tier-local schemas.
> - Every enum-like reference table in any tier MUST follow the canonical `(Id, Code, Label)` shape per **Rule 13** ([`02-spec/04-database-conventions/03-schema-design.md`](../04-database-conventions/03-schema-design.md) §6.5). The legacy `{Table}Code` / `{Table}Label` column-name style is removed.
...
>
> - **Main / Worker service** — see [`02-spec/19-main-worker-service/12-split-db-tier-reconciliation.md`](../19-main-worker-service/12-split-db-tier-reconciliation.md). Main = 3 tiers (Root/Settings/Session); Worker = 4 tiers (Root/Settings/App/Session). Cache + Document unused in v1.0.
