...
**Prevention:** All log context array keys must use camelCase. See [naming-conventions.md § Array Key Conventions](../../01-spec-authoring-guide/03-naming-conventions.md).
...
**Prevention:** Rule 4 in [code-style.md](./04-code-style/01-index.md). Exception: if `return`/`throw` is the only statement, no blank line needed.
