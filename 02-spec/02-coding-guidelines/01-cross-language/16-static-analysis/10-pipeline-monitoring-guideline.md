...
```
Exports the failure trace from GitHub Actions directly into `.ai-memory/temp/ci-failure.json` (or the configured temp folder in settings) so AI subagents can parse the failure root cause.
...
flowchart TD
    ...
    F --> G[Analyze Failure RCA in .ai-memory/temp/ci-failure.json]
...
gitmap ui pipeline
```
Configures `.ai-memory/temp` folder paths, polling intervals, and terminal drawer preferences directly in SQLite and localStorage.
