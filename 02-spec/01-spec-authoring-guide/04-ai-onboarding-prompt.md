...
|-------|------|----------------|
| 1 | `.ai-memory/overview.md` | Project summary, tech stack, navigation map |
...
| 4 | `.ai-memory/memory/index.md` | Index of all institutional knowledge files |
| 5 | `.ai-memory/plan.md` | Current active roadmap and priorities |
| 6 | `.ai-memory/suggestions.md` | Pending improvement ideas (not yet approved) |

### Step 1.2 — Read EVERY file referenced in `.ai-memory/memory/index.md`
...
| The `.ai-memory/` folder structure and its purpose | `07-memory-folder-guide.md` |
...
│  └─ YES → Write to `.ai-memory/memory/` and update `.ai-memory/memory/index.md`
│
...
- When adding a new memory file, **always** update the index at `.ai-memory/memory/index.md`.
...
- **Quick refresh** — For an AI that has already been onboarded, ask it to re-read only `.ai-memory/strictly-avoid.md` and `.ai-memory/memory/index.md`
...
- `.ai-memory/overview.md` — Shorter always-present onboarding doc
