...
| R-5 | Patterns matching `/API/V1/Workers/*` or `/API/V1/SelfUpdate` are LOCKED — PATCH returns `403 EndpointAuthLocked` (`MAIN-400-10` / flat `21170`, cataloged in `13-error-codes.md` §3.4 — FU-18 RESOLVED 2026-05-05). These endpoints are always-protected per §8. | `EndpointPathPattern` |
...
| `CorrelationId` | The inbound `X-Correlation-Id` header. The middleware MUST reject the PATCH if absent (per `02-spec/04-database-conventions/06-rest-api-format.md`). |
...
- Header conventions: `02-spec/04-database-conventions/06-rest-api-format.md` §X-Auth-Action
