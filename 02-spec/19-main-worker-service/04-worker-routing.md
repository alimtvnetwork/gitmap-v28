...
The single source of truth for every routing-related endpoint (paths, methods, request/response bodies, auth, idempotency, error codes) is **`06-core-api-endpoints.md`**. This file MUST NOT redefine them. A literal AI implementer MUST read `06-` and ignore any earlier draft of an endpoint catalog here.

| Concern | Authoritative location |
|---|---|
| Endpoint paths + auth column | `06-` §2 (Endpoint Catalog) |
| Request / response payload shapes | `06-` §3 (Reference Payloads) and the `21-backup-endpoints.md` family for Backup-tier |
| Per-endpoint idempotency rules | `06-` §1 + `15-tunable-constants.md` §2.2 |
| Header contract (`X-Correlation-Id`, `X-Idempotency-Key`, `X-Auth-Action`) | `02-spec/04-database-conventions/06-rest-api-format.md` |
