...
The envelope shape from §2 is **bidirectional**. Workers also use it when reporting failures back to Main on Worker-initiated calls — namely Heartbeat (`POST /Workers/Heartbeat`), Register (`POST /Workers/Register`, see `10-worker-bootstrap-protocol.md`), and Push-Update acknowledgement (`POST /Workers/PushAck`, see `02-spec/14-update/28-worker-push-instruction.md`). (Resolves F-A-32.)
...
- `02-spec/04-database-conventions/06-rest-api-format.md` — `X-Correlation-Id`, `X-Idempotency-Key`, `X-Auth-Action` header conventions (single source of truth)
