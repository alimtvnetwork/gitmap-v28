...
**Spec under audit:** `02-spec/19-main-worker-service/` (24 numbered files, ~7,016 lines)
...
**Fix (one sentence).** Promote the inferred answer to §6.4 as a MUST: *"Stage 4 MUST NOT issue per-envelope `PRAGMA journal_mode=WAL` — the App tier's session-wide WAL (per `02-spec/05-split-db-architecture/`) is authoritative."*
