...
> **Spec under audit:** `02-spec/19-main-worker-service/` (29 numbered files, ~370 KB).
...
`02-spec/19-main-worker-service/diagrams/` was not refreshed in Phase 13.2. The Mermaid/PNG diagrams still show:
...
`11-` now declares reconciliation **applied**, but several rules it inherits from `02-spec/01-app/` and `02-spec/03-tasks/` are still **referenced by link**, not inlined. A blind AI handed only the `02-spec/19-…` folder cannot resolve them.
...
2. **Golden-file fixtures**: add `02-spec/19-main-worker-service/fixtures/` with one request/response per endpoint in `06-`, one per error code family in `13-`, one JWT example in `12-` → +7.
...
**Grade: B (75 / 100).** The spec moved from **operationally hostile** (F) to **operationally tractable** (B) in one hardening pass. The remaining gaps are documentation craftsmanship — fixtures, diagrams, three missed seeds — not structural ambiguity. A blind AI handed the current `02-spec/19-…/` folder will ship a working main-worker service with predictable, reviewable wire-format drift, instead of a broken core.
