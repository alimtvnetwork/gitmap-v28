...
> **Spec under audit:** `02-spec/19-main-worker-service/` (29 numbered files, ~370 KB / ~7,000 lines, excluding `98-changelog.md`).
...
|---|------|-------------------|--------------|
| M-1 | `00-§7` | `02-spec/03-error-manage/`, other sibling specs | Inherited rules can't be read → AI ignores them. |
...
5. **Inline the externals**: copy the inherited rules from `02-spec/03-error-manage/` into `08-`/`13-`; define the caching policy locally in `15-` (drop the `mem://` URI from `01-`).
...
**Recommendation:** treat the seven edits in §6 as the v5.22.0 hardening pass. They are mechanical, ≤ 15 line-touches each, and close the entire critical category. After they land, re-run `bun run spec:audit` and confirm the score crosses 90 for `02-spec/19-main-worker-service`.
...
# Inputs: every *.md under 02-spec/19-main-worker-service/ except 98-changelog.md
