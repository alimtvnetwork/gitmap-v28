...
> **Spec under audit:** `02-spec/19-main-worker-service/` (28 numbered + 4 meta + audit/diagrams/images/fixtures dirs).
...
> **Method:** (a) full re-run of every linter against v5.43.0 corpus; (b) hostile re-read pass on the non-backup surface (00–17, 24–25); (c) physical PNG-vs-`.mmd` coverage check inside `02-spec/19/diagrams/`; (d) regression check that v5.42.0 → v5.43.0 changes (renderer output only) did not perturb spec markdown.
...
| v5.43.0 | `@mermaid-js/mermaid-cli@11.4.2` added as dev dep; `node scripts/render-diagrams.mjs` executed; 20 new PNGs committed (8 inside `02-spec/19/diagrams/`). | Closes 8/9 of audit-12 §2.2; +1 Testability (visual artifact now reproducible from committed source), +1 Blind-buildability (literal AI reading PNGs no longer sees out-of-band stale visuals for the covered set). |
...
`02-spec/19-main-worker-service/diagrams/` — 9 `.mmd` sources, 8 PNGs committed:
...
Plus `02-spec/19/images/` ships 4 topology PNGs (no `.mmd` sources — hand-drawn assets), all present.
