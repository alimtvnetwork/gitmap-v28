...
| F-A-38 | `Seedable-Config` (current spec assumes reader knows) | many | MINOR — link to `02-spec/06-`. |
...
| 4 | `04-cross-spec-dependency-audit.md` | Verify `02-spec/03/04/05/06/14` anchors actually exist |
...
**Method:** For each finding, look up the closing task in `.ai-memory/question-and-ambiguity/task-counter.md` and verify by file inspection.
...
| F-A-22 | BLOCKER | ✅ CLOSED | #14 | `02-spec/04-database-conventions/06-rest-api-format.md` X-Auth-Action authoritative |
