...
**Spec audited:** `02-spec/19-main-worker-service/`
...
| F-N-06 | `09-self-update-pointer.md` references `02-spec/14-update/` — not validated against actual existence in step 1 | (step 4 will check) |
...
| 4 | `04-cross-spec-dependency-audit.md` | Verify referenced `02-spec/05/06/14/04` anchors exist |
...
| F-B-06 | `/Workers/.../Update` request body missing | ✅ | `02-spec/14-update/28-worker-push-instruction.md` (task #07) + `06-…md` §2.5 patch (task #29) |
...
| F-N-06 | `02-spec/14-update/` cross-ref validation | ✅ | Verified in cross-spec audit (task #33) |
| F-N-07 | No OpenAPI artifact mentioned | ⏸ | Deferred — tooling concern, post-v1.0 |
| F-N-08 | Timestamp precision unspecified | ✅ | `02-spec/04-database-conventions/01-naming-conventions.md` Rule 7.1: `YYYY-MM-DDTHH:MM:SS.sssZ`, ms + UTC `Z` mandatory. Logged in `98-changelog.md`. Task previously shipped. |
