...
| `09-self-update-pointer.md` | Pointer to `02-spec/14-update/` + bounded sunset + deletion checklist | 1.2.0 | ✅ |
...
rg -i -n \
   --glob '!02-spec/19-main-worker-service/02-glossary.md' \
   ...
   --glob '!02-spec/19-main-worker-service/plan.md' \
...
- **Phase 5** (separate task): bump `package.json` minor, run `node scripts/sync-version.mjs` and `node scripts/sync-spec-tree.mjs`, log entry in `.ai-memory/plan.md`.
