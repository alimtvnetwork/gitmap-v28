...
Both Main and Worker consume **Seedable-Config** (per `02-spec/06-seedable-config-architecture/`). For terminology mapping (including replaced legacy terms), see `02-glossary.md` §Reserved.
...
- Every `catch` logs per `02-spec/03-error-manage/` and the inline contract in `08-error-contract.md`.
