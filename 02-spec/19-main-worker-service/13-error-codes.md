...
| `WORKER-403-02` | `21045` | `PayloadHostNotAllowed` | "`PayloadUrl` host not on `AllowedHostsAllowlist` (Channel C defense-in-depth)." | 403 | `16-update-channels.md` §C |
...
| `WORKER-500-02` | `21051` | `HandoffFailed` | "Post-update handoff did not confirm within `WorkerPushUpdate.HandoffTimeoutSeconds` (per `15-tunable-constants.md` §2.7, default 60 s); rolled back." | 500 | `02-spec/14-update/28` §6 |
...
| Code | Flat | Name | Message | HTTP | Source |
|---|---|---|---|---|---|
| `MAIN-300-01` | `21130` | `CorrelationIdMissing` | "Required `X-Correlation-Id` header absent." | 400 | `04-worker-routing.md` §7.4 + `02-spec/04-database-conventions/06-rest-api-format.md` |
...
| `MAIN-400-07` | `21149` | `WorkerPushAckUnknownJid` | "PushAck received for unknown / expired Job-Id; Worker logs and discards." | 404 | `08-error-contract.md` §9 + `02-spec/14-update/28-worker-push-instruction.md` |
...
2. **R2 No orphans** — every code cataloged here MUST be referenced from ≥1 source location outside the catalog files (`13-error-codes.md`, `error-codes.json`, `error-codes-master.json`). Codes documented only by range-notation cross-reference (e.g. "`WORKER-100-01..05`") may be waived in §7.1.
...
| `WORKER-500-02`, `WORKER-500-03` | `02-spec/14-update/28` §6 + `11-split-db-tier-reconciliation.md` | range mention |
...
| `MAIN-100-01..04` | `05-auth-and-2fa.md` | catalog-only sub-range |
| `MAIN-200-01`, `MAIN-200-02` | `06-core-api-endpoints.md` §2.5/§2.7, `07-role-based-dashboards.md` | catalog-only |
| `MAIN-400-02`, `MAIN-400-03` | `04-worker-routing.md` §3.1 | range "WorkerUnreachable / quarantine" |
| `MAIN-500-01` | `03-main-db-schema.md` | catalog-only |
...
- `02-spec/19-main-worker-service/08-error-contract.md` — JSON envelope shape.
- `02-spec/19-main-worker-service/10-worker-bootstrap-protocol.md` §6 — bootstrap codes referenced.
- `02-spec/19-main-worker-service/12-jwt-delivery-contract.md` §7 — JWT verification codes referenced.
