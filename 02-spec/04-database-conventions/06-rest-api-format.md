...
| `X-Idempotency-Key` | All `POST`, `PUT`, `PATCH` | ULID (26 chars), max 64 chars | Server stores result for `IdempotencyKeyTtlSeconds` (per `02-spec/19-main-worker-service/15-tunable-constants.md` §2.2). Replay returns cached result. |
...
- `02-spec/19-main-worker-service/06-core-api-endpoints.md` §1 — first introduced these headers.
- `02-spec/19-main-worker-service/15-tunable-constants.md` §2.2 — TTL + max-length values.
- `02-spec/19-main-worker-service/13-error-codes.md` — error codes for header-validation failures.
