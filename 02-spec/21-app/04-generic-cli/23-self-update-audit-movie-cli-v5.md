...
**Reference**: gitmap-v28 repo (this codebase), `gitmap-v28/cmd/update.go`,
`gitmap-v28/cmd/updatescript.go`, `run.ps1` (`-Update` branch).
...
language) that mirror `gitmap-v28/cmd/update.go` lines 18–157. Do **not** keep
...
Reference: `02-spec/08-generic-update/06-cleanup.md`.
...
check it first. See `gitmap-v28/cmd/update.go` `resolveRepoPath()`.
...
See `gitmap-v28/cmd/updatescript.go::writeScriptToTemp` (BOM = `0xEF 0xBB 0xBF`).
...
4. **Implement Go `movie update` + hidden `movie update-runner`** mirroring
   `gitmap-v28/cmd/update.go`. Use foreground `cmd.Run()` (D1, D9).
...
- Read `gitmap-v28/cmd/update.go`, `gitmap-v28/cmd/updatescript.go`, and the
...
- Do **not** invent alternative strategies (named pipes, scheduled tasks,
  ...
  `02-spec/02-app-issues/02-update-flow-spec-alignment.md`.
- Every PR touching update flow MUST update all of:
  `02-spec/03-general/02f-self-update-orchestration.md`,
  `02-spec/04-generic-cli/22-self-update-gold-standard.md`,
  `02-spec/08-generic-update/*`, and the audited project's own checklist.
