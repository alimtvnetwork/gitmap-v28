...
**Status:** Fixed in v2.84.0 — user-visible failure mode further softened in v6.0.x (see `02-spec/02-app-issues/34-hd-hosted-docs-fallback.md`: `gitmap hd` now falls back to hosted docs at `constants.DocsURL` instead of hard-exiting when local `docs-site/` is missing).
