...
### A. Immediate Cause
The conditional `if err != nil` was placed inside the `if idx >= 0 && idx < len(item.Recipes)` block, resulting in a nest depth of 2. Repository coding standard `02-spec/02-coding-guidelines/02-anti-patterns/01-nested-ifs.md` strictly bans any nested `if` statements (nest depth $\ge 2$).
...
3. **CI Issue Ledger Logging:**
   All CI pipeline breakages must be logged in `02-spec/22-app-issues/` and `02-spec/12-cicd-pipeline-workflows/` with reproducible symptoms and durable prevention steps.
