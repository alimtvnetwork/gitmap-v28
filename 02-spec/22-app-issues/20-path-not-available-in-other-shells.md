...
After installing gitmap-v28 via `curl | sh` on macOS (zsh default), the binary
...
3. gitmap-v28 works in zsh terminals.
4. User opens `sh` or switches to bash → `gitmap-v28: command not found`.
...
1. **Installer Output Contract** (in `.ai-memory/plan.md`): Unix installers
...
- `02-spec/02-app-issues/19-missing-macos-binaries-and-lint-regression.md`
- `.ai-memory/plan.md` — Installer Output Contract guardrail
